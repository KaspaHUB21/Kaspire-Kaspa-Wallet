...
| AUTH | Partial | Every sign, secret export and wallet deletion is authorized inside Kotlin using Android BiometricPrompt/device credentials or a throttled Kaspire PIN on supported Android 11–16 devices. Native code—not Dart—selects the prompt text; transaction/KCC20/reveal prompts are built from the Rust-prepared review cached under its hash, and KIP-5 signing displays the exact address and message in a native review dialog. A random 20-second, one-use capability is bound to the exact operation and review hash/message/wallet, then atomically consumed before touching the secret. The vault key remains deliberately not auth-bound because several OEM Keystores failed authenticated `Cipher.init`; independent review of that compatibility tradeoff remains open. |
...
| CODE | Partial | Rust dependencies are locked and the native signer fails closed. Website releases use a dedicated 4096-bit RSA Android release key kept outside the repository; v0.10.12 introduces it through an Android signing-certificate lineage from the legacy certificate. Flutter/Gradle dependencies are locked locally, but reproducible-build verification, SBOM generation, vulnerability scanning and release minification review remain open. |
...
- Physical-device matrix covering supported Android 11–16 biometric/no-biometric devices,
  gesture/three-button navigation, low-memory process death, tablets and foldables.
- Reproducible APK/AAB verification and dependency/SBOM review.
