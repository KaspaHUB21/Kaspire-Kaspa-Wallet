...
try {
  ...
  await waitFor('document.querySelectorAll(".asset-group").length===4');
  ...
  assert.deepEqual(ordered.map(x=>x.values),[['BURTLE','KASBTC','ZULU'],['ALPHA','BURTLE','ZETA'],['ANGELS','BURTLE','ZOMBIES'],['alpha.kas','beta.kas','zulu.kas']]);
  ...
  await shot('send');
  await cdp('Page.reload');await waitFor('document.querySelectorAll(".asset-group").length===4');
