...
impl<'a, 'i> ForLowerer<'a, 'i> {
    ...
    fn lower_for_statement(
        ...
        let max_iterations = match eval_const_int(max_iterations, self.constants) {
            Ok(value) => value,
            Err(CompilerError::InvalidLiteral(message)) => {
                return Err(CompilerError::InvalidLiteral(message));
            }
            Err(_) => {
                return Err(CompilerError::Unsupported(
                    "for loop max iterations must be a compile-time integer".to_string(),
                ));
            }
        };
        if max_iterations < 0 {
        ...
        if let (Ok(start_value), Ok(end_value)) = (
            eval_const_int(start, self.constants),
            eval_const_int(end, self.constants),
        ) {
        ...
        let accumulator_type_ref = TypeRef {
            base: TypeBase::Int,
            array_dims: Vec::new(),
        };
    ...
    fn lower_constant_for_statement(
        ...
        if i128::from(end) - i128::from(start) > max_iterations as i128 {
            return Err(CompilerError::Unsupported(
                "for loop range must not exceed max iterations".to_string(),
            ));
        }

        let lowered_body = self.lower_block(body)?;
        let loop_var_type_ref = TypeRef {
            base: TypeBase::Int,
            array_dims: Vec::new(),
        };
        let mut lowered = Vec::new();
