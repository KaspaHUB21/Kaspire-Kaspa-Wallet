...
use super::*;
use crate::ast::{
    ArrayDim, ConstantAst, ContractAst, ContractFieldAst, FunctionAst, ParamAst, Statement, TypeRef,
};
...
pub(super) fn lower_inferred_array_sizes<'i>(
    ...
    let functions_by_name: HashMap<String, &FunctionAst<'i>> = contract
        .functions
        .iter()
...
fn lower_constant<'i>(
    ...
    .ok_or_else(|| {
        CompilerError::Unsupported(format!(
            "cannot infer fixed array size from constant '{}'",
            constant.name
        ))
    })?;
...
fn lower_field<'i>(
    ...
    .ok_or_else(|| {
        CompilerError::Unsupported(format!(
            "cannot infer fixed array size from contract field '{}'",
            field.name
        ))
    })?;
    ...
    match statement {
        Statement::VariableDefinition {
            ...
            let lowered_type = infer_type_ref(type_ref, expr.as_ref(), types, constants, functions)
                .ok_or_else(|| {
                    CompilerError::Unsupported(format!(
                        "cannot infer fixed array size from variable '{}'",
                        name
                    ))
                })?;
        ...
        Statement::FunctionCallAssign {
            ...
            let lowered_bindings = bindings
                ...
                .map(|binding| {
                    let lowered_type =
                        infer_type_ref(&binding.type_ref, None, types, constants, functions)
                            .ok_or_else(|| {
                                CompilerError::Unsupported(format!(
                                    "cannot infer fixed array size from binding '{}'",
                                    binding.name
                                ))
                            })?;
...
fn infer_type_ref<'i>(
    ...
    if matches!(declared_type.array_size(), Some(ArrayDim::Inferred)) {
        infer_fixed_array_type_from_initializer_ref(
            declared_type,
            initializer,
            types,
            constants,
...
fn infer_expr_type_ref<'i>(
    ...
    match &expr.kind {
        ...
        ExprKind::Array(values) => {
            let mut inferred = array_literal_element_type.cloned().or_else(|| {
                infer_array_literal_element_type(values, types, constants, functions)
            })?;
        ...
        ExprKind::Binary {
            ...
            inferred
                .array_dims
                .push(ArrayDim::Fixed(left_size.checked_add(right_size)?));
            Some(inferred)
        }
        ExprKind::IfElse {
            then_expr,
        ...
        ExprKind::Append { source, args, .. } => {
            ...
            inferred
                .array_dims
                .push(ArrayDim::Fixed(source_size.checked_add(args.len())?));
            Some(inferred)
        }
        ExprKind::UnarySuffix {
            source,
            kind: UnarySuffixKind::Reverse,
            ..
...
fn infer_array_literal_element_type<'i>(
    ...
    if values.iter().skip(1).all(|value| {
        infer_expr_type_ref(value, types, constants, functions, None).as_ref() == Some(&first_type)
    }) {
        Some(first_type)
    } else {
        None
