...
use crate::ast::{
    BinaryOp, Expr, ExprKind, IntrospectionKind, NullaryOp, TypeBase, UnaryOp, UnarySuffixKind,
};
...
fn builtin_call_value_type(name: &str) -> &'static str {
    match name {
        ...
        | "OpCovOutputCount"
        | "OpCovOutputIdx" => "int",
        "OpTxInputIsCoinbase" | "checkSig" | "checkSigFromStack" | "checkSigFromStackECDSA" => {
            "bool"
        }
        "blake2b" | "blake2bWithKey" | "blake3" | "blake3WithKey" | "templateHash" | "sha256"
        | "OpSha256" => "byte[32]",
        "bytes"
        | "OpTxSubnetId"
        | "OpTxPayloadSubstr"
...
pub(super) fn infer_debug_expr_value_type<'i>(
    ...
    match &expr.kind {
        ...
        ExprKind::Split { .. } | ExprKind::Slice { .. } | ExprKind::New { .. } => {
            Ok("byte[]".to_string())
        }
        ExprKind::Append { source, .. } => {
            infer_debug_expr_value_type(source, env, types, visiting)
        }
        ...
        ExprKind::FieldAccess { .. } => Err(CompilerError::Unsupported(
...
mod tests {
    ...
    fn infer(
        expr: Expr<'static>,
        env: HashMap<String, Expr<'static>>,
        types: HashMap<String, String>,
    ) -> String {
    ...
    fn infers_literal_and_identifier_value_types() {
        ...
        assert_eq!(
            infer(Expr::bool(true), HashMap::new(), HashMap::new()),
            "bool"
        );
        assert_eq!(
            infer(Expr::byte(0xaa), HashMap::new(), HashMap::new()),
            "byte"
        );
        assert_eq!(
            infer(Expr::string("hi"), HashMap::new(), HashMap::new()),
            "string"
        );
    ...
    fn infers_addition_and_array_index_value_types() {
        ...
        let index = Expr::new(
            ExprKind::ArrayIndex {
                source: Box::new(Expr::identifier("items")),
    ...
    fn infers_known_builtin_and_unknown_call_value_types() {
        ...
        assert_eq!(
            infer(input_covenant_id, HashMap::new(), types.clone()),
            "byte[32]"
        );
    ...
    fn infers_nullary_introspection_and_suffix_value_types() {
        let nullary = Expr::new(
            ExprKind::Nullary(NullaryOp::ActiveScriptPubKey),
            span::Span::default(),
        );
