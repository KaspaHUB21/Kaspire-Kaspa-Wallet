...
pub(super) fn static_check_contract<'i>(
    ...
    if contract.params.len() != constructor_args.len() {
        return Err(CompilerError::Unsupported(
            "constructor argument count mismatch".to_string(),
        ));
    }
    ...
    let constants: HashMap<String, Expr<'i>> = contract
        .constants
        .iter()
    ...
    for (param, value) in contract.params.iter().zip(constructor_args.iter()) {
        ...
        if !expr_matches_declared_type_ref(value, &param.type_ref, &structs) {
            return Err(CompilerError::Unsupported(format!(
                "constructor argument '{}' expects {}",
                param.name, param_type_name
            )));
        }
    }
...
fn validate_pragma_versions<'i>(contract: &ContractAst<'i>) -> Result<(), CompilerError> {
    ...
    let compiler_version = Version::parse(COMPILER_VERSION).map_err(|err| {
        CompilerError::Unsupported(format!(
            "invalid SilverScript compiler version '{COMPILER_VERSION}': {err}"
        ))
    })?;
    if pragma.name != "silverscript" {
        return Err(
            CompilerError::Unsupported(format!("unknown pragma '{}'", pragma.name))
                .with_span(&pragma.name_span),
        );
    }
    let req = VersionReq::parse(&pragma.value).map_err(|err| {
        CompilerError::Unsupported(format!(
            "invalid SilverScript version requirement '{}': {err}",
            pragma.value
        ))
...
pub(crate) fn validate_return_types<'i>(
    ...
    if return_types.is_empty() {
        return Err(CompilerError::Unsupported(
            "return requires function return types".to_string(),
        ));
    }
    if return_types.len() != exprs.len() {
        return Err(CompilerError::Unsupported(
            "return values count must match function return types".to_string(),
        ));
    }
    for (expr, return_type) in exprs.iter().zip(return_types.iter()) {
        if !expr_matches_return_type_ref(expr, return_type, types, structs, constants) {
            ...
            return Err(CompilerError::Unsupported(format!(
                "return value expects {type_name}"
            )));
        }
    }
    Ok(())
...
fn validate_contract_struct_usage<'i>(
    contract: &ContractAst<'i>,
    structs: &StructRegistry,
) -> Result<(), CompilerError> {
...
fn validate_function_signatures<'i>(
    ...
    for function in &contract.functions {
        for param in &function.params {
            ensure_array_elements_have_known_size(
                &param.type_ref,
                structs,
                &type_name_from_ref(&param.type_ref),
            )?;
        }
        ...
        if function.entrypoint
            && !options.allow_entrypoint_return
            && !function.return_types.is_empty()
        {
            return Err(CompilerError::Unsupported(
                "entrypoint return requires allow_entrypoint_return=true".to_string(),
            ));
        }
        ...
        if has_return {
            if !matches!(function.body.last(), Some(Statement::Return { .. })) {
                return Err(CompilerError::Unsupported(
                    "return statement must be the last statement".to_string(),
                ));
            }
            if function.body[..function.body.len() - 1]
                .iter()
...
fn validate_contract_field_initializers<'i>(
    ...
    for field in &contract.fields {
        ...
        validate_expr_assignable_to_type(
            &field.expr,
            &field.type_ref,
            &types,
            structs,
    ...
    for stmt in statements {
        match stmt {
            ...
            Statement::TupleAssignment {
                ...
                expr,
                ..
            } => validate_tuple_assignment_statement_shape(
                &mut ctx,
                left_type_ref,
                left_name,
                right_type_ref,
            ...
            Statement::Assign { name, expr, .. } => {
                validate_assign_statement_shape(&mut ctx, name, expr)?
            }
...
fn validate_variable_definition_statement_shape<'i>(
    ...
    let effective_type_ref = infer_fixed_array_type_from_initializer_type_check(
        type_ref,
        expr,
        ctx.types,
        ctx.constants,
    ...
    match expr {
        Some(Expr {
            ...
            Some(other_type) => match parse_type_ref(other_type) {
                Ok(other_type_ref)
                    if is_type_assignable_ref(&other_type_ref, type_ref, constants) =>
                {
                    Ok(())
                }
                Ok(_) => Err(CompilerError::Unsupported(
                    "array assignment requires compatible array types".to_string(),
                )),
                Err(_) => Err(CompilerError::Unsupported(
                    "array assignment requires compatible array types".to_string(),
                )),
            },
        ...
        None => Err(CompilerError::Unsupported(
            "variable definition requires initializer".to_string(),
        )),
    }
}
...
fn validate_state_function_call_assign_statement_shape<'i>(
    ...
    for binding in bindings {
        ensure_array_elements_have_known_size(
            &binding.type_ref,
            ctx.structs,
            &type_name_from_ref(&binding.type_ref),
        )?;
...
fn validate_function_call_assign_statement_shape<'i>(
    ...
    if bindings.len() != function.return_types.len() {
        return Err(CompilerError::Unsupported(
            "function call assignment return count mismatch".to_string(),
        ));
    }
...
fn validate_struct_destructure_bindings<'i>(
    ...
    let struct_name = struct_name_from_type_ref(&expr_type, structs).ok_or_else(|| {
        CompilerError::Unsupported("struct destructuring requires a struct value".to_string())
    })?;
    let struct_ast = structs
        .get(struct_name)
        .ok_or_else(|| CompilerError::Unsupported(format!("unknown struct '{struct_name}'")))?;
    let direct_read_input_state =
        matches!(&expr.kind, ExprKind::Call { name, .. } if name == "readInputState");
    ...
    if bindings.len() != struct_ast.fields.len() {
        return Err(CompilerError::Unsupported(
            "struct destructuring must bind all fields exactly once".to_string(),
        ));
    }
    ...
    for field in &struct_ast.fields {
        ...
        if direct_read_input_state && is_struct(&binding.type_ref, structs) {
            return Err(CompilerError::Unsupported(
                "readInputState does not support nested struct fields".to_string(),
            ));
        }
    }
...
fn validate_expr_semantics<'i>(
    ...
    match &expr.kind {
        ExprKind::Binary { op, left, right } => {
            ...
            let left_value_type = super::debug_value_types::infer_debug_expr_value_type(
                left,
                env,
                types,
                &mut HashSet::new(),
            ...
            .ok();
            let right_value_type = super::debug_value_types::infer_debug_expr_value_type(
                right,
                env,
                types,
                &mut HashSet::new(),
            ...
            {
                return Err(CompilerError::Unsupported(
                    "byte values do not support '+'".to_string(),
                ));
            }
            if matches!(
                op,
                ...
                let coerced_right =
                    coerce_rhs_byte_literal_for_comparison_ref(left_type.as_ref(), right);
        ...
        ExprKind::Unary { expr, .. } => validate_expr_semantics(
            expr,
            env,
            prefer_env_for_comparison,
            types,
        ...
        ExprKind::Call { name, args, .. } => {
            ...
            if let Some(function) = functions.get(name) {
                if function.entrypoint {
                    return Err(CompilerError::Unsupported(format!(
                        "entrypoint function '{}' cannot be called",
                        name
                    )));
                }
        ...
        ExprKind::Append { source, args, .. } => {
            ...
            .ok_or_else(|| {
                CompilerError::Unsupported("append target must be an array".to_string())
            })?;
            let Some(element_type) = source_type.array_element_type() else {
                return Err(CompilerError::Unsupported(
                    "append target must be an array".to_string(),
                ));
            };
            for arg in args {
                validate_expr_semantics(
        ...
        ExprKind::Introspection { index, .. } => validate_expr_semantics(
            index,
            env,
            prefer_env_for_comparison,
            types,
        ...
        ExprKind::UnarySuffix { source, .. } => validate_expr_semantics(
            source,
            env,
            prefer_env_for_comparison,
            types,
...
fn infer_expr_type_ref_for_comparison_ref<'i>(
    ...
    match &expr.kind {
        ExprKind::Identifier(name) => {
            ...
            {
                let type_name = super::debug_value_types::infer_debug_expr_value_type(
                    value,
                    env,
                    types,
                    &mut HashSet::new(),
        ...
        ExprKind::Append { source, .. } => infer_expr_type_ref_for_comparison_ref(
            source,
            env,
            prefer_env_for_comparison,
            types,
...
fn tuple_field_index(field: &str) -> Option<usize> {
    (!field.is_empty() && field.chars().all(|ch| ch.is_ascii_digit()))
        .then(|| field.parse().ok())
        .flatten()
}
...
fn validate_tuple_field_access<'i>(
    ...
    let ExprKind::Call { name, args, .. } = &source.kind else {
        return Err(CompilerError::Unsupported(
            "tuple field access requires a tuple-returning function call".to_string(),
        ));
    };
    for arg in args {
        validate_expr_semantics(
...
fn coerce_rhs_byte_literal_for_comparison_ref<'i>(
...
fn comparison_types_compatible_ref(left_type: &TypeRef, right_type: &TypeRef) -> bool {
    ...
    matches!(
        (
            &left_type.base,
            left_type.array_dims.as_slice(),
            &right_type.base,
            right_type.array_dims.as_slice()
        ),
...
fn insert_type_binding(
        ...
        && let Ok(leaves) = flatten_type_ref_leaves(type_ref, structs)
    {
        for (path, leaf_type) in leaves {
            types.insert(
                flattened_struct_name(name, &path),
                type_name_from_ref(&leaf_type),
            );
        }
    }
    Ok(())
...
fn validate_builtin_call<'i>(
    ...
    for (arg, &(arg_name, expected_type_name)) in args.iter().zip(expected_args) {
        ...
        .ok_or_else(|| {
            CompilerError::Unsupported(format!(
                "{name}() argument '{arg_name}' expects {expected_type_name}"
            ))
        })?;
        if !is_type_assignable_ref(&actual_type, &expected_type, constants)
            && !expr_matches_type_ref(arg, &expected_type)
        {
            return Err(CompilerError::Unsupported(format!(
...
fn validate_expr_assignable_to_type<'i>(
    ...
    if let ExprKind::FieldAccess { field, .. } = &expr.kind
        ...
        && let Some(actual_type) = infer_expr_type_ref_for_comparison_ref(
            expr,
            &HashMap::new(),
            &HashSet::new(),
            types,
    ...
    if is_struct(type_ref, structs) {
        ...
        {
            return compile::read_input_state_with_template_values(
                args, type_ref, structs, constants,
            )
            .map(|_| ());
        }
        ...
        let matches = match &expr.kind {
            ...
            .is_some_and(|source_type| {
                ...
                match (
                    ...
                    (Some(source_size), Some(expected_size)) => {
                        source_size.checked_add(args.len()) == Some(expected_size)
                    }
                    _ => false,
                }
            }),
        ...
        if type_ref.is_array()
            && let Ok(actual_type_name) = super::debug_value_types::infer_debug_expr_value_type(
                expr,
                &HashMap::new(),
                types,
                &mut HashSet::new(),
...
fn ensure_array_elements_have_known_size(
    ...
    if type_ref.is_array()
        && fixed_type_size_ref(
            type_ref.array_element_type().as_ref().unwrap_or(type_ref),
            structs,
        )
        .is_none()
    {
...
fn fixed_type_size_ref(type_ref: &TypeRef, structs: &StructRegistry) -> Option<i64> {
    ...
    .and_then(|base_size| {
        type_ref
            .array_dims
            .iter()
            .try_fold(base_size, |acc, dim| match dim {
    ...
    match stmt {
        ...
        Statement::If {
            ...
            then_branch.iter().any(statement_contains_return)
                || else_branch
                    .as_ref()
                    .is_some_and(|branch| branch.iter().any(statement_contains_return))
        }
...
pub(crate) fn expr_matches_declared_type_ref<'i>(
    expr: &Expr<'i>,
    type_ref: &TypeRef,
    structs: &StructRegistry,
) -> bool {
...
pub(super) fn expr_matches_return_type_ref<'i>(
    ...
    match &expr.kind {
        ExprKind::IfElse { .. } => {
            if let Ok(actual_type_name) = super::debug_value_types::infer_debug_expr_value_type(
                expr,
                &HashMap::new(),
                types,
                &mut HashSet::new(),
        ...
        | ExprKind::Bool(_)
        | ExprKind::Byte(_)
        | ExprKind::String(_) => expr_matches_type_ref(expr, type_ref),
...
pub(super) fn array_literal_matches_type_ref<'i>(values: &[Expr<'i>], type_ref: &TypeRef) -> bool {
    ...
    values
        .iter()
        .all(|value| expr_matches_type_ref(value, &element_type))
}
...
fn expr_matches_return_type_ref_hint<'i>(
    ...
    if validate_expr_assignable_to_type(
        expr,
        type_ref,
        types,
        structs,
    ...
    match (&expr.kind, &type_ref.base, !type_ref.is_array()) {
        (ExprKind::Array(values), TypeBase::Byte, true) if values.len() == 1 => {
            match values[0].kind {
                ExprKind::Byte(byte) => Some(format!(
                    "hex literals are byte arrays; use byte({byte:#04x}) to cast a one-byte hex literal to byte"
                )),
                _ => None,
            }
        }
