...
use super::*;
use crate::ast::{
    ConstantAst, ContractAst, ContractFieldAst, Expr, ExprKind, FunctionAst, ParamAst,
    STATE_TYPE_NAME, Statement, StructBindingAst, TypeBase, TypeRef, parse_type_ref,
};
use crate::span;
...
pub(crate) fn build_struct_registry<'i>(
    contract: &ContractAst<'i>,
) -> Result<StructRegistry, CompilerError> {
    ...
    for item in &contract.structs {
        ...
        if registry
            .insert(item.name.clone(), StructSpec { fields })
            .is_some()
        {
            return Err(CompilerError::Unsupported(format!(
...
pub(crate) fn struct_name_from_type_ref<'a>(
    type_ref: &'a TypeRef,
...
pub(crate) fn struct_array_name_from_type_ref(
    type_ref: &TypeRef,
...
pub(crate) fn ensure_known_or_builtin_type(
    type_ref: &TypeRef,
    structs: &StructRegistry,
    context: &str,
    ...
    if !type_ref.is_array() {
        ...
        if structs.contains_key(name) {
            return Err(CompilerError::Unsupported(format!(
                "arrays of struct type '{}' are not supported",
                name
            )));
        }
        return Err(CompilerError::Unsupported(format!(
...
pub(crate) fn validate_struct_graph(structs: &StructRegistry) -> Result<(), CompilerError> {
    fn visit(
        ...
        if !visiting.insert(name.to_string()) {
            return Err(CompilerError::Unsupported(format!(
                "cyclic struct definition involving '{name}'"
            )));
        }
        let item = structs
            .get(name)
...
pub(crate) fn resolve_struct_access<'i>(
    ...
    match &expr.kind {
        ...
        ExprKind::FieldAccess { source, field, .. } => {
            ...
            let struct_name =
                struct_name_from_type_ref(&current_type, structs).ok_or_else(|| {
                    CompilerError::Unsupported("field access requires a struct value".to_string())
                })?;
            let item = structs.get(struct_name).ok_or_else(|| {
        ...
        _ => Err(CompilerError::Unsupported(
            "struct field access requires a struct variable".to_string(),
        )),
    }
}
...
fn lower_expr<'i>(
    ...
    match &expr.kind {
        ExprKind::FieldAccess { source, field, .. } => {
            ...
            {
                let (base, mut path, array_type) =
                    resolve_struct_access(array_source, scope, structs)?;
            ...
            if is_struct(&type_ref, structs) {
                return Err(CompilerError::Unsupported(
                    "struct value must be used in a struct-typed position".to_string(),
                ));
            }
            Ok(Expr::new(
                ExprKind::Identifier(flattened_struct_name(&base, &path)),
                span,
            ))
        }
        ExprKind::Unary { op, expr } => Ok(Expr::new(
            ExprKind::Unary {
                op: *op,
                expr: Box::new(lower_expr(expr, scope, structs)?),
            },
            span,
        )),
        ...
        ExprKind::Array(values) => Ok(Expr::new(
            ExprKind::Array(
                values
                    .iter()
                    .map(|value| lower_expr(value, scope, structs))
                    .collect::<Result<Vec<_>, _>>()?,
            ),
            span,
        )),
        ExprKind::StructLiteral(_) => Err(CompilerError::Unsupported(
            "struct literals are only supported in struct-typed positions".to_string(),
        )),
        ExprKind::Call {
            name,
            args,
        ...
        ExprKind::UnarySuffix {
                ...
                && is_struct_array(type_ref, structs)
            {
                let first_leaf = flatten_type_ref_leaves(type_ref, structs)?
                    ...
                    .next()
                    .ok_or_else(|| {
                        CompilerError::Unsupported("struct array must contain fields".to_string())
                    })?;
                return Ok(Expr::new(
                    ExprKind::UnarySuffix {
                        source: Box::new(Expr::identifier(flattened_struct_name(
                            name,
                            &first_leaf.0,
                        ))),
                        kind: *kind,
...
pub(crate) fn lower_struct_value_expr<'i>(
    ...
    let expected_struct_name =
        struct_name_from_type_ref(expected_type, structs).ok_or_else(|| {
            CompilerError::Unsupported(format!(
...
pub(crate) fn lower_struct_destructure_statement<'i>(
    ...
    for binding in bindings {
        ...
        debug_assert!(
            replaced.is_none(),
            "type_check must validate duplicate struct destructuring fields"
        );
    }
    ...
    for field in &struct_ast.fields {
        let binding = binding_map
            .remove(&field.name)
            .expect("type_check must validate exact struct destructuring coverage");
        debug_assert_eq!(
            binding.type_ref, field.type_ref,
            "type_check must validate struct destructuring field types"
        );

        if let Some(field_expr) = direct_field_values
            .as_ref()
            .and_then(|fields| fields.get(&field.name))
        {
            ...
            scope
                .vars
                .insert(binding.name.clone(), binding.type_ref.clone());
            ...
            if is_struct(&binding.type_ref, structs) {
                ...
                scope
                    .vars
                    .insert(binding.name.clone(), binding.type_ref.clone());
                lowered.extend(paths.into_iter().zip(lowered_values).map(
                    |((path, field_type), field_expr)| Statement::VariableDefinition {
                        type_ref: field_type,
                        modifiers: Vec::new(),
    ...
    debug_assert!(
        ...
        "type_check must validate exact struct destructuring coverage"
    );
...
pub(crate) fn lowering_scope_from_types(
    types: &HashMap<String, String>,
) -> Result<LoweringScope, CompilerError> {
    ...
    match &expr.kind {
        ExprKind::Identifier(name) => {
            ...
            Ok(leaves
                ...
                .map(|(path, _)| {
                    Expr::new(
                        ExprKind::Identifier(flattened_struct_name(name, &path)),
                        span::Span::default(),
                    )
                })
                .collect())
        ...
        ExprKind::Array(values) => {
            let element_type = expected_type.array_element_type().ok_or_else(|| {
                CompilerError::Unsupported(format!(
                    "expected struct type '{}'",
                    expected_type.type_name()
                ))
            ...
            let mut grouped: Vec<Vec<Expr<'i>>> =
                vec![Vec::with_capacity(values.len()); leaf_specs.len()];
            for value in values {
            ...
            Ok(grouped
                .into_iter()
                .map(|entries| Expr::new(ExprKind::Array(entries), span::Span::default()))
                .collect())
        }
...
fn lower_value_for_named_type<'i>(
    ...
    if is_struct(type_ref, structs) || is_struct_array(type_ref, structs) {
        ...
        Ok(flatten_type_ref_leaves(type_ref, structs)?
            .into_iter()
            .zip(lowered)
            .map(|((path, leaf_type), lowered_expr)| {
                (flattened_struct_name(name, &path), leaf_type, lowered_expr)
            })
            .collect())
    } else {
        Ok(vec![(
            name.to_string(),
            type_ref.clone(),
            lower_runtime_expr(expr, &scope_types, structs)?,
        )])
    }
}
    ...
    for stmt in statements {
        match stmt {
            ...
            Statement::VariableDefinition {
                ...
                if is_struct(type_ref, structs) || is_struct_array(type_ref, structs) {
                    if let Some(Expr {
                        ...
                        && matches!(
                            builtin_name.as_str(),
                            "readInputState" | "readInputStateWithTemplate"
                        )
                        ...
                        && item.fields.iter().all(|field| {
                            !is_struct(&field.type_ref, structs)
                                && !is_struct_array(&field.type_ref, structs)
                        })
                    {
                    ...
                    if let Some(expr) = expr {
                        ...
                        for (leaf_name, leaf_type) in
                            flatten_named_type_like(name, type_ref, structs)?
                        {
                            lowered.push(Statement::VariableDefinition {
                                type_ref: leaf_type,
                                modifiers: modifiers.clone(),
                    ...
                    lowered.push(Statement::VariableDefinition {
                        ...
                        expr: expr
                            .as_ref()
                            .map(|expr| lower_expr(expr, scope, structs))
                            .transpose()?,
                        span: *span,
                        type_span: *type_span,
                        modifier_spans: modifier_spans.clone(),
            ...
            Statement::TupleAssignment {
                ...
                scope
                    .vars
                    .insert(right_name.clone(), right_type_ref.clone());
            ...
            Statement::Assign {
                ...
                if is_struct(&type_ref, structs) || is_struct_array(&type_ref, structs) {
                    ...
                    {
                        let element_type = type_ref.array_element_type().ok_or_else(|| {
                            CompilerError::Unsupported(
                                "array element type not supported".to_string(),
                            )
                        })?;
                        ...
                        for arg in args {
                            for ((path, _leaf_type), leaf_expr) in
                                flatten_type_ref_leaves(&element_type, structs)?
                                    .into_iter()
...
fn initial_contract_scope<'i>(contract: &ContractAst<'i>) -> LoweringScope {
    ...
    for param in &contract.params {
        scope
            .vars
            .insert(param.name.clone(), param.type_ref.clone());
    }
    for constant in &contract.constants {
        scope
            .vars
            .insert(constant.name.clone(), constant.type_ref.clone());
    }
    for field in &contract.fields {
        scope
            .vars
            .insert(field.name.clone(), field.type_ref.clone());
    }
    scope
}
...
pub(crate) fn flatten_constructor_args_env<'i>(
    ...
    for (param, arg) in params.iter().zip(constructor_args.iter()) {
        for (name, _, expr) in lower_value_for_named_type(
            param.name.as_str(),
            &param.type_ref,
            arg,
            &scope,
