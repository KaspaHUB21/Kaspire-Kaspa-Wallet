...
use super::*;
use crate::ast::{
    ContractAst, Expr, ExprKind, FunctionAst, STATE_TYPE_NAME, StateFieldExpr, Statement, TypeBase,
    TypeRef,
};
use crate::span;
...
/// ```
pub(super) fn lower_read_input_state_calls<'i>(
...
fn lower_statement<'i>(
    ...
    let lowered = match statement {
        Statement::VariableDefinition {
            ...
            expr: expr
                .as_ref()
                .map(|expr| lower_expr(expr, &mut prefix, context)),
            span: *span,
            type_span: *type_span,
        ...
        Statement::Block { body, span } => Statement::Block {
        ...
        Statement::If {
            ...
            else_branch: else_branch
                .as_ref()
                .map(|branch| lower_block(branch, context)),
            span: *span,
            then_span: *then_span,
            else_span: *else_span,
        },
...
fn lower_call_args<'i>(
    args: &[Expr<'i>],
    prefix: &mut Vec<Statement<'i>>,
    context: &mut LoweringContext,
) -> Vec<Expr<'i>> {
    args.iter()
        .map(|arg| lower_call_arg(arg, prefix, context))
        .collect()
}
...
fn lower_expr<'i>(
    ...
    let kind = match &expr.kind {
        ExprKind::Array(values) => ExprKind::Array(
            values
                .iter()
                .map(|value| lower_expr(value, prefix, context))
                .collect(),
        ),
        ExprKind::Call {
            name,
        ...
        ExprKind::Unary { op, expr } => ExprKind::Unary {
            op: *op,
            expr: Box::new(lower_expr(expr, prefix, context)),
        },
