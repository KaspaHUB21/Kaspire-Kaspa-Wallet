...
use crate::ast::{
    ContractAst, Expr, ExprKind, FunctionAst, StateFieldExpr, Statement, TypeBase, TypeRef,
};
...
fn lower_statements<'i>(
    ...
    for stmt in statements {
        match stmt {
            ...
            {
                let lowered_expr = coerce_expr_for_declared_scalar_type(
                    substitute_expr(expr, &local_aliases)?,
                    type_ref,
                )?;
            ...
            Statement::VariableDefinition {
                ...
                lowered.push(Statement::VariableDefinition {
                    ...
                    expr: expr
                        .as_ref()
                        .map(|expr| substitute_expr(expr, &local_aliases))
                        .transpose()?,
                    span: *span,
                    type_span: *type_span,
            ...
            Statement::FunctionCall {
                ...
                args: args
                    .iter()
                    .map(|arg| substitute_expr(arg, &local_aliases))
                    .collect::<Result<Vec<_>, _>>()?,
                span: *span,
                name_span: *name_span,
            }),
            ...
            Statement::If {
                ...
                else_branch: else_branch
                    .as_ref()
                    .map(|branch| {
                        lower_statements(branch, assigned_names, identifier_uses, &local_aliases)
                    })
                    .transpose()?,
                span: *span,
                then_span: *then_span,
...
fn coerce_expr_for_declared_scalar_type<'i>(
    expr: Expr<'i>,
    type_ref: &TypeRef,
    ...
    {
        let byte_value = value.try_into().map_err(|_| {
            CompilerError::Unsupported(format!("integer literal {value} is out of range for byte"))
        })?;
...
fn substitute_expr<'i>(
    expr: &Expr<'i>,
    aliases: &HashMap<String, Expr<'i>>,
) -> Result<Expr<'i>, CompilerError> {
    let Expr { kind, span } = expr.clone();
    Ok(match kind {
        ExprKind::Identifier(name) => aliases
            .get(&name)
            .cloned()
            .unwrap_or_else(|| Expr::new(ExprKind::Identifier(name), span)),
        ExprKind::Unary { op, expr } => Expr::new(
            ExprKind::Unary {
                op,
                expr: Box::new(substitute_expr(&expr, aliases)?),
            },
            span,
        ),
        ...
        ExprKind::Array(values) => Expr::new(
            ExprKind::Array(
                values
                    .iter()
                    .map(|value| substitute_expr(value, aliases))
                    .collect::<Result<Vec<_>, _>>()?,
            ),
            span,
        ),
        ...
        ExprKind::Call {
            ...
            ExprKind::Call {
                ...
                args: args
                    .iter()
                    .map(|arg| substitute_expr(arg, aliases))
                    .collect::<Result<Vec<_>, _>>()?,
                name_span,
            },
            span,
...
fn expr_references_any(expr: &Expr<'_>, names: &HashSet<String>) -> bool {
    match &expr.kind {
        ...
        ExprKind::Append { source, args, .. } => {
            expr_references_any(source, names)
                || args.iter().any(|arg| expr_references_any(arg, names))
        }
        ExprKind::IfElse {
            condition,
            then_expr,
        ...
        ExprKind::StructLiteral(fields) => fields
            .iter()
            .any(|field| expr_references_any(&field.expr, names)),
        ExprKind::Call { args, .. } | ExprKind::New { args, .. } => {
            args.iter().any(|arg| expr_references_any(arg, names))
        }
        ExprKind::Split { source, index, .. } | ExprKind::ArrayIndex { source, index } => {
