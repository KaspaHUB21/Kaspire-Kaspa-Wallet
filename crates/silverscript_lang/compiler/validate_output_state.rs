...
use super::*;
use crate::ast::{
    ContractAst, Expr, ExprKind, FunctionAst, STATE_TYPE_NAME, Statement, TypeBase, TypeRef,
};
use crate::span;

pub(super) const VALIDATE_OUTPUT_STATE_INNER: &str = "__validateOutputStateInner";
pub(super) const VALIDATE_OUTPUT_STATE_WITH_TEMPLATE_INNER: &str =
    "__validateOutputStateWithTemplateInner";
...
pub(super) fn lower_validate_output_state<'i>(
    ...
    let functions = contract
        .functions
        .iter()
        .map(|function| lower_function(function, contract, structs))
...
fn lower_function<'i>(
    ...
    Ok(FunctionAst {
        body: lower_statements(&function.body, &mut scope, structs)?,
        ..function.clone()
    })
}
...
fn lower_statement<'i>(
    ...
    match statement {
        Statement::FunctionCall {
            ...
            let Ok([output_idx, state_expr]): Result<&[Expr<'i>; 2], _> =
        ...
        Statement::FunctionCall {
            ...
            ): Result<&[Expr<'i>; 5], _> = args.as_slice().try_into()
            else {
                return Err(CompilerError::Unsupported(
            ...
            let mut lowered_args = vec![output_idx.clone()];
            lowered_args.extend(flatten_struct_expr(
                &state_expr,
                &state_type,
                scope,
                structs,
            ...
            lowered_args.extend([
                ...
                template_suffix.clone(),
                expected_template_hash.clone(),
            ]);
            lowered.push(Statement::FunctionCall {
        ...
        Statement::Block { body, span } => {
            ...
            Ok(vec![Statement::Block {
                body: lower_statements(body, &mut block_scope, structs)?,
                span: *span,
            }])
        }
        Statement::If {
...
fn infer_template_state_type(
    expr: &Expr<'_>,
    scope: &ValidationScope,
    structs: &StructRegistry,
) -> Result<TypeRef, CompilerError> {
    let struct_scope = super::structs::LoweringScope {
...
fn flatten_struct_expr<'i>(
    ...
    if !matches!(&expr.kind, ExprKind::Identifier(_)) {
        return Err(CompilerError::Unsupported(
            "flatten_struct_expr expects an identifier".into(),
        ));
    }
