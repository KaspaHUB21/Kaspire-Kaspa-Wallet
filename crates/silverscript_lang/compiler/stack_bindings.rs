...
impl StackBindings {
    ...
    pub(crate) fn from_order(ordered_names: Vec<String>) -> Self {
        ...
        assert_eq!(
            input_len,
            stack.len(),
            "stack binding order should not contain duplicates"
        );
        Self { stack }
    }
    ...
    pub(crate) fn from_depths(depths: HashMap<String, i64>) -> Self {
        ...
        assert!(
            ordered
                .iter()
                .enumerate()
                .all(|(expected_depth, (_, depth))| *depth == expected_depth as i64),
            "illegal stack binding depths"
        );
        Self {
            stack: ordered.into_iter().map(|(name, _)| name).collect(),
        }
    }
    ...
    pub(crate) fn clone_depths(&self) -> HashMap<String, i64> {
        self.stack
            ...
            .cloned()
            .enumerate()
            .map(|(depth, name)| (name, depth as i64))
            .collect()
    }
    ...
    pub(crate) fn insert_binding(&mut self, name: &str, depth: i64) {
        assert!(
            (0..=self.stack.len() as i64).contains(&depth),
            "depth out of bounds: {depth}"
        );
        let target_index = depth as usize;
        assert!(
            self.stack.insert_before(target_index, name.to_string()).1,
            "binding already exists: {name}"
        );
    }
    ...
    fn remove_binding(&mut self, name: &str) {
        assert!(
            ...
            "removed binding {name} should exist"
        );
    }
    ...
    fn move_binding_to_top(&mut self, name: &str) {
        let from = self
            .stack
            .get_index_of(name)
            .expect("binding should exist before moving to top");
    ...
    /// `ROLL+DROP` strategy.
    pub(crate) fn emit_drop_bindings(
        &mut self,
        names: &[String],
        ...
        for name in self.binding_order() {
            ...
            let depth = self
                .depth(&name)
                .expect("binding should exist before dropping");
    ...
    pub(crate) fn emit_update_stack_for_rebinding(
        &mut self,
        name: &str,
        builder: &mut ScriptBuilder,
    ) -> Result<(), CompilerError> {
        let depth = self
            .depth(name)
            .expect("binding should exist before stack rebinding");
    ...
    #[allow(dead_code)]
    pub(crate) fn emit_move_binding_to_top(
        &mut self,
        name: &str,
        builder: &mut ScriptBuilder,
    ) -> Result<(), CompilerError> {
    ...
    pub(crate) fn emit_stack_reordering(
        &self,
        target_bindings: &Self,
        builder: &mut ScriptBuilder,
    ) -> Result<(), CompilerError> {
        ...
        for name in move_prefix {
            let index = remaining_stack
                .get_index_of(name)
                .expect("binding existence was asserted above");
        ...
        debug_assert!(
            remaining_stack
                .iter()
                .eq(target_bindings.stack[move_prefix.len()..].iter())
        );
...
impl Permutation {
    ...
    fn from_orders(current_order: &StackBindings, target_order: &StackBindings) -> Self {
        assert!(
            current_order.set_eq(target_order),
            "stack reconciliation requires both layouts to contain the same bindings"
        );
        Self {
            indices: target_order
                .stack
                .iter()
                .map(|name| current_order.stack.get_index_of(name).expect("set equal"))
                .collect(),
        }
    }
    ...
    fn apply_to<T: Clone>(&self, canonical_order: &[T]) -> Vec<T> {
        self.indices
            .iter()
            .map(|&index| canonical_order[index].clone())
            .collect()
    }
}
...
mod tests {
    ...
    fn execute_script_and_decode_stack(script: Vec<u8>) -> Vec<i64> {
        ...
        let stacks =
            TxScriptEngine::<PopulatedTransaction, SigHashReusedValuesUnsync>::from_script(
                &script,
                &reused_values,
                &sig_cache,
                EngineFlags {
    ...
    fn execute_local_opcode_sequence_top_to_bottom(
        values_top_to_bottom: &[i64],
        opcodes: &[u8],
    ) -> Vec<i64> {
    ...
    fn rebinding_moves_name_to_top_and_shifts_shallower_bindings() {
        ...
        stack_bindings
            .emit_update_stack_for_rebinding("b", &mut builder)
            .expect("rebind stack slot");
    ...
    fn drop_bindings_uses_drop_for_top_and_roll_for_deeper_entries() {
        ...
        stack_bindings
            .emit_drop_bindings(&names(&["a", "c"]), &mut builder)
            .expect("drop selected bindings");
    ...
    fn stack_reordering_uses_local_swap_when_available() {
        ...
        stack_bindings
            .emit_stack_reordering(&target_bindings, &mut builder)
            .expect("reorder with swap");
    ...
    fn stack_reordering_uses_suffix_rebuild_for_non_local_permutation() {
        ...
        current_bindings
            .emit_stack_reordering(&target_bindings, &mut builder)
            .expect("reorder with suffix rebuild");
        ...
        script_builder
            .add_ops(&script)
            .expect("append reordering ops");

        assert_eq!(
            execute_script_and_decode_stack(script_builder.drain()),
            vec![4, 5, 3, 2, 1]
        );
    }
    ...
    fn local_stack_reordering_search_finds_two_op_sequence() {
        ...
        for opcode in opcodes {
            reordered = reordered
                ...
                .expect("planned local opcode should apply");
        }
    ...
    fn apply_local_opcode_matches_stack_machine_rotation_direction() {
        assert_eq!(
            Permutation::identity(3)
                .apply_local_opcode(OpRot)
                .map(|p| p.indices),
            Some(vec![2, 0, 1])
        );
        assert_eq!(
            Permutation::identity(4)
                .apply_local_opcode(Op2Swap)
                .map(|p| p.indices),
            Some(vec![2, 3, 0, 1])
        );
        assert_eq!(
            Permutation::identity(6)
                .apply_local_opcode(Op2Rot)
                .map(|p| p.indices),
            Some(vec![4, 5, 0, 1, 2, 3])
        );
    ...
    fn local_stack_reordering_and_emit_match_canonical_one_or_two_op_sequences() {
        ...
        let initial_order = (0..initial_values.len())
            .map(|index| format!("v{index}"))
            .collect::<Vec<_>>();
        let value_to_label = initial_values
            .iter()
            .copied()
            .zip(initial_order.iter().cloned())
            .collect::<HashMap<_, _>>();
        ...
        for source_opcodes in local_opcode_sequences_in_search_order() {
            ...
            let target_values =
                execute_local_opcode_sequence_top_to_bottom(&initial_values, &source_opcodes);
            let target_order = target_values
                .iter()
                .map(|value| {
                    value_to_label
                        .get(value)
                        .expect("target value should map to initial label")
                        .clone()
                })
                .collect::<Vec<_>>();
    ...
    fn emitted_stack_reordering_matches_engine_execution() {
        ...
        stack_bindings
            .emit_stack_reordering(&target_bindings, &mut reorder_builder)
            .expect("emit stack reordering");
    ...
    fn emitted_rebinding_matches_engine_execution() {
        ...
        stack_bindings
            .emit_update_stack_for_rebinding("b", &mut rebinding_builder)
            .expect("emit rebinding update");
        ...
        script
            .add_ops(&rebinding_builder.drain())
            .expect("append rebinding ops");
