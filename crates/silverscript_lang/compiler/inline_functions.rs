...
impl<'i, 'd> Inliner<'i, 'd> {
        ...
        match statement {
            Statement::VariableDefinition {
                ...
                let renamed_expr = if let Some(expr) = expr {
                    let (prelude, renamed_expr) =
                        self.lower_expr(expr, scope, visited_functions)?;
            ...
            Statement::FunctionCall {
                ...
                if let Some(function) = self.inline_target(name) {
                    lowered.extend(self.inline_call(
                        &function,
                        args,
                        None,
                        scope,
                    ...
                    let (prelude, renamed_args) =
                        self.lower_exprs(args, scope, visited_functions)?;
                    lowered.extend(prelude);
                    self.push_lowered_statement(
                        &mut lowered,
                        Statement::FunctionCall {
            ...
            Statement::If {
                ...
                let (prelude, renamed_condition) =
                    self.lower_expr(condition, scope, visited_functions)?;
                lowered.extend(prelude);
                let mut then_scope = scope.clone();
                let lowered_then =
                    self.lower_block(then_branch, &mut then_scope, visited_functions)?;
            ...
            Statement::For {
                ...
                let (mut prelude, lowered_start) =
                    self.lower_expr(start, scope, visited_functions)?;
                let (more_prelude, lowered_end) = self.lower_expr(end, scope, visited_functions)?;
                prelude.extend(more_prelude);
                let (more_prelude, lowered_max_iterations) =
                    self.lower_expr(max_iterations, scope, visited_functions)?;
                prelude.extend(more_prelude);
                lowered.extend(prelude);
                self.push_lowered_statement(
                    &mut lowered,
    ...
    fn inline_target(&self, name: &str) -> Option<FunctionAst<'i>> {
        self.functions
            .get(name)
            .cloned()
            .filter(|function| !function.entrypoint) // TODO: Store this information in a separate set for efficiency
    }
    ...
    fn inline_call(
        ...
        let mut lowered = Vec::new();
        self.debug_recorder
            .begin_inline_source_call(&function.name, SourceSpan::from(span));
        ...
        for (param, arg) in function.params.iter().zip(args.iter()) {
            ...
            lowered.extend(prelude);
            self.debug_recorder.record_inline_source_param(
                &param.name,
                &param.type_ref,
                renamed_arg.clone(),
            );
        ...
        if let (Some(bindings), Some(return_exprs)) = (bindings, return_exprs) {
            for (binding, expr) in bindings.iter().zip(return_exprs.iter()) {
                let (prelude, renamed_expr) =
                    self.lower_expr(expr, &local_scope, visited_functions)?;
                lowered.extend(prelude);
                self.push_lowered_statement(
                    &mut lowered,
                    Statement::VariableDefinition {
        ...
        self.debug_recorder
            .finish_inline_source_call(body_end_statement_index);
        Ok(lowered)
    }
    ...
    fn lower_expr(
        ...
        match &expr.kind {
            ...
            ExprKind::String(value) => {
                Ok((Vec::new(), Expr::new(ExprKind::String(value.clone()), span)))
            }
            ExprKind::DateLiteral(value) => {
                Ok((Vec::new(), Expr::new(ExprKind::DateLiteral(*value), span)))
            }
            ExprKind::Identifier(name) => Ok((
                Vec::new(),
                Expr::new(ExprKind::Identifier(self.rename_name(name, scope)), span),
            )),
            ...
            ExprKind::Call {
                ...
                if let Some(function) = self.inline_target(name) {
                    ...
                    prelude.extend(self.inline_call(
                        &function,
                        &args,
                        Some(&[binding]),
                        scope,
            ...
            ExprKind::IfElse {
                ...
                let (mut prelude, condition) =
                    self.lower_expr(condition, scope, visited_functions)?;
                let (more_prelude, then_expr) =
                    self.lower_expr(then_expr, scope, visited_functions)?;
                prelude.extend(more_prelude);
                let (more_prelude, else_expr) =
                    self.lower_expr(else_expr, scope, visited_functions)?;
                prelude.extend(more_prelude);
                Ok((
                    prelude,
                    Expr::new(
            ...
            ExprKind::StructLiteral(fields) => {
                ...
                for field in fields {
                    let (field_prelude, expr) =
                        self.lower_expr(&field.expr, scope, visited_functions)?;
                ...
                Ok((
                    prelude,
                    Expr::new(ExprKind::StructLiteral(lowered_fields), span),
                ))
            }
            ExprKind::FieldAccess {
                    ...
                    && let Some(function) = self.inline_target(name)
                {
                    ...
                    let temp_names = function
                        .return_types
                        .iter()
                        .map(|_| self.fresh_name(name))
                        .collect::<Vec<_>>();
                    ...
                    self.debug_recorder.record_visible_name(
                        &selected_name,
                        &format!("{}.{}", function.name, index),
                    );
            ...
            ExprKind::NumberWithUnit { value, unit } => Ok((
                Vec::new(),
                Expr::new(
    ...
    fn push_lowered_statement(
        &mut self,
        lowered: &mut Vec<Statement<'i>>,
        statement: Statement<'i>,
    ) {
        self.debug_recorder
            .record_lowered_source_statement(&statement);
