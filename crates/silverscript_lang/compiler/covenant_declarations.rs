    ...
    for function in &contract.functions {
        ...
        match declaration.binding {
            CovenantBinding::Auth => {
                ...
                let mut wrapper = build_auth_wrapper(
                    &policy,
                    &policy_name,
                    declaration.clone(),
                    entrypoint_name,
                ...
                wrapper.params =
                    preserved_entrypoint_params(function, declaration, true, &contract.fields);
                lowered.push(wrapper);
            }
            CovenantBinding::Cov => {
                let leader_name = generated_covenant_leader_entrypoint_name(&function.name);
                let mut leader_wrapper = build_cov_wrapper(
                    &policy,
                    &policy_name,
                    declaration.clone(),
                    leader_name,
                ...
                leader_wrapper.params = preserved_entrypoint_params(
                    function,
                    declaration.clone(),
                    true,
                    &contract.fields,
                ...
                let mut delegate_wrapper = build_cov_wrapper(
                    &policy,
                    &policy_name,
                    declaration.clone(),
                    delegate_name,
                ...
                delegate_wrapper.params =
                    preserved_entrypoint_params(function, declaration, false, &contract.fields);
...
fn parse_covenant_declaration<'i>(
    ...
    if function.attributes.len() != 1 {
        return Err(CompilerError::Unsupported(
            "covenant declarations support exactly one #[covenant(...)] attribute".to_string(),
        ));
    }
    ...
    for arg in &attribute.args {
        if args_by_name.insert(arg.name.as_str(), &arg.expr).is_some() {
            return Err(CompilerError::Unsupported(format!(
                "duplicate covenant attribute argument '{}'",
                arg.name
            )));
        }
    }

    let allowed_keys: HashSet<&str> = ["binding", "from", "to", "mode", "groups", "termination"]
        .into_iter()
        .collect();
    for arg in &attribute.args {
        if !allowed_keys.contains(arg.name.as_str()) {
            return Err(CompilerError::Unsupported(format!(
                "unknown covenant attribute argument '{}'",
                arg.name
            )));
        }
    }
    ...
    let (from_expr, to_expr) = match syntax {
            ...
            let from_expr = args_by_name
                ...
                .ok_or_else(|| {
                    CompilerError::Unsupported(
                        "missing covenant attribute argument 'from'".to_string(),
                    )
                })?
                .clone();
        ...
        CovenantSyntax::Singleton => {
            if args_by_name.contains_key("from") || args_by_name.contains_key("to") {
                return Err(CompilerError::Unsupported(
                    "covenant.singleton is sugar and does not accept 'from' or 'to' arguments"
                        .to_string(),
                ));
            }
        ...
        CovenantSyntax::Fanout => {
            ...
            let to_expr = args_by_name
                ...
                .ok_or_else(|| {
                    CompilerError::Unsupported(
                        "missing covenant attribute argument 'to'".to_string(),
                    )
                })?
                .clone();
    ...
    let from_value = eval_const_int(&from_expr, constants).map_err(|_| {
        CompilerError::Unsupported("covenant 'from' must be a compile-time integer".to_string())
    })?;
    if from_value < 1 {
        return Err(CompilerError::Unsupported(
            "covenant 'from' must be >= 1".to_string(),
        ));
    }

    let default_binding = if from_value == 1 {
        CovenantBinding::Auth
    } else {
        CovenantBinding::Cov
    };
    let binding = match args_by_name.get("binding").copied() {
        Some(expr) => {
            ...
            match binding_name.as_str() {
                ...
                other => {
                    return Err(CompilerError::Unsupported(format!(
                        "covenant binding must be auth|cov, got '{}'",
                        other
                    )));
                }
            }
    ...
    let mode = match args_by_name.get("mode").copied() {
        Some(expr) => {
            ...
            match mode_name.as_str() {
                ...
                other => {
                    return Err(CompilerError::Unsupported(format!(
                        "covenant mode must be verification|transition, got '{}'",
                        other
                    )));
                }
            }
    ...
    let groups = match args_by_name.get("groups").copied() {
        Some(expr) => {
            ...
            match groups_name.as_str() {
                ...
                other => {
                    return Err(CompilerError::Unsupported(format!(
                        "covenant groups must be single|multiple, got '{}'",
                        other
                    )));
                }
            }
    ...
    if binding == CovenantBinding::Auth && from_value != 1 {
        return Err(CompilerError::Unsupported(
            "binding=auth requires from = 1".to_string(),
        ));
    }
    if binding == CovenantBinding::Cov
        && is_literal_int(&from_expr, 1)
        && args_by_name.contains_key("binding")
    {
        eprintln!(
    ...
    if binding == CovenantBinding::Cov && groups == CovenantGroups::Multiple {
        return Err(CompilerError::Unsupported(
            "binding=cov with groups=multiple is not supported yet".to_string(),
        ));
    }

    if mode == CovenantMode::Verification && !function.return_types.is_empty() {
        return Err(CompilerError::Unsupported(
            "verification mode policy functions must not declare return values".to_string(),
        ));
    }
    if mode == CovenantMode::Transition && function.return_types.is_empty() {
        return Err(CompilerError::Unsupported(
            "transition mode policy functions must declare return values".to_string(),
        ));
    }

    let infers_single_state_transition_to_one = mode == CovenantMode::Transition
        && function.return_types.len() == 1
        && is_state_type_ref(&function.return_types[0]);
    ...
    let to_value = eval_const_int(&to_expr, constants).map_err(|_| {
        CompilerError::Unsupported("covenant 'to' must be a compile-time integer".to_string())
    })?;
    if to_value < 1 {
        return Err(CompilerError::Unsupported(
            "covenant 'to' must be >= 1".to_string(),
        ));
    }

    if args_by_name.contains_key("termination") && !(from_value == 1 && to_value == 1) {
        return Err(CompilerError::Unsupported(
            "termination is only supported for singleton covenants (from=1, to=1)".to_string(),
        ));
    }
...
fn parse_attr_ident_arg<'i>(name: &str, value: Option<&Expr<'i>>) -> Result<String, CompilerError> {
    let value = value.ok_or_else(|| {
        CompilerError::Unsupported(format!("missing covenant attribute argument '{}'", name))
    })?;
    match &value.kind {
        ExprKind::Identifier(identifier) => Ok(identifier.clone()),
        _ => Err(CompilerError::Unsupported(format!(
            "covenant attribute argument '{}' must be an identifier",
            name
        ))),
    }
}
...
fn validate_covenant_policy_state_shape<'i>(
    ...
    if declaration.mode == CovenantMode::Transition {
        ...
        if declaration.singleton
            && is_state_array_type_ref(return_type)
            && declaration.termination != CovenantTermination::Allowed
        {
            return Err(CompilerError::Unsupported(format!(
...
fn preserved_entrypoint_params<'i>(
    ...
    match (declaration.binding, declaration.mode, leader) {
        ...
        (CovenantBinding::Cov, CovenantMode::Verification, true) => {
            function.params.iter().skip(1).cloned().collect()
        }
        (CovenantBinding::Cov, CovenantMode::Transition, true) => {
            function.params.iter().skip(1).cloned().collect()
        }
...
fn build_auth_wrapper<'i>(
    ...
    body.push(var_def_statement(
        int_type_ref(),
        auth_out_count_name,
        Expr::call("OpAuthOutputCount", vec![active_input.clone()]),
    ));
    ...
    if declaration.groups == CovenantGroups::Single {
        ...
        body.push(var_def_statement(
            bytes32_type_ref(),
            cov_id_name,
            Expr::call("OpInputCovenantId", vec![active_input.clone()]),
        ));
    ...
    if !contract_fields.is_empty() {
            ...
            CovenantMode::Verification => {
                ...
                body.push(call_statement(
                    ...
                    policy
                        .params
                        .iter()
                        .map(|param| identifier_expr(&param.name))
                        .collect(),
                ));
                if declaration.singleton && declaration.termination != CovenantTermination::Allowed
                {
                    ...
                    body.push(call_statement(
                        "validateOutputState",
                        vec![
                            identifier_expr(out_idx_name),
                            identifier_expr(new_state_name),
                        ],
                    ));
                } else {
            ...
            CovenantMode::Transition => {
                ...
                if is_state_type_ref(&policy.return_types[0]) {
                    ...
                    body.push(call_statement(
                        ...
                        vec![
                            identifier_expr(out_idx_name),
                            identifier_expr(next_state_name),
                        ],
                    ));
                } else {
        ...
        let call_args: Vec<Expr<'i>> = policy
            .params
            .iter()
            .map(|param| identifier_expr(&param.name))
            .collect();
...
fn build_cov_wrapper<'i>(
    ...
    let leader_idx_expr = Expr::call(
        "OpCovInputIdx",
        vec![identifier_expr(cov_id_name), Expr::int(0)],
    );
    body.push(require_statement(binary_expr(
        if leader { BinaryOp::Eq } else { BinaryOp::Ne },
        leader_idx_expr,
        active_input,
    )));
    ...
    if leader {
        ...
        body.push(var_def_statement(
            int_type_ref(),
            in_count_name,
            Expr::call("OpCovInputCount", vec![identifier_expr(cov_id_name)]),
        ));
        body.push(require_statement(binary_expr(
            BinaryOp::Le,
            identifier_expr(in_count_name),
        ...
        if !contract_fields.is_empty() {
            ...
            match declaration.mode {
                ...
                CovenantMode::Transition => {
                    return Err(CompilerError::Unsupported(
                        "mode=tranisition is not supported when contract state is empty"
                            .to_string(),
                    ));
                }
            }
...
fn bytes32_type_ref() -> TypeRef {
    TypeRef {
        base: TypeBase::Byte,
        array_dims: vec![ArrayDim::Fixed(32)],
    }
}

fn active_input_index_expr<'i>() -> Expr<'i> {
    Expr::new(
        ExprKind::Nullary(NullaryOp::ActiveInputIndex),
        span::Span::default(),
    )
}
...
fn array_index_expr<'i>(name: &str, index: Expr<'i>) -> Expr<'i> {
    Expr::new(
        ExprKind::ArrayIndex {
            source: Box::new(identifier_expr(name)),
            index: Box::new(index),
        },
        span::Span::default(),
    )
...
fn array_append_statement<'i>(name: &str, expr: Expr<'i>) -> Statement<'i> {
    Statement::Assign {
        ...
        expr: Expr::new(
            ExprKind::Append {
                source: Box::new(Expr::identifier(name)),
                args: vec![expr],
                span: span::Span::default(),
            },
            span::Span::default(),
...
fn state_object_expr_from_contract_fields<'i>(
    contract_fields: &[ContractFieldAst<'i>],
) -> Expr<'i> {
...
fn append_cov_input_state_reads_into_state_array<'i>(
    ...
    let for_body = vec![array_append_statement(
        ...
        Expr::call(
            ...
            vec![Expr::call(
                "OpCovInputIdx",
                vec![identifier_expr(cov_id_name), identifier_expr(loop_var)],
            )],
        ),
    )];
...
fn append_auth_output_state_array_checks_from_state_array<'i>(
    ...
    let for_body = vec![
        var_def_statement(
            ...
            Expr::call(
                "OpAuthOutputIdx",
                vec![active_input.clone(), identifier_expr(loop_var)],
            ),
        ),
        call_statement(
            "validateOutputState",
            vec![
                identifier_expr(out_idx_name),
                array_index_expr(state_array_name, identifier_expr(loop_var)),
            ],
        ),
    ];
    body.push(for_statement(
