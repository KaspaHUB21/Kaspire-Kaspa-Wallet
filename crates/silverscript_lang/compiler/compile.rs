...
pub(super) fn read_input_state_field_expr_symbolic<'i>(
    ...
    let state_start_offset = state_start_offset(
        contract_field_prefix_len,
        contract_fields,
        contract_constants,
    )?;
    let script_size_expr = Expr::new(
        ExprKind::Nullary(NullaryOp::ThisScriptSize),
        span::Span::default(),
    );
    let field_payload_len = fixed_state_field_payload_len(field, contract_constants)?;
    let field_payload_offset =
        state_start_offset + field_chunk_offset + data_prefix(field_payload_len).len();
    ...
    let start = Expr::new(
        ExprKind::Binary {
            ...
            left: Box::new(Expr::new(
                ExprKind::Binary {
                    op: BinaryOp::Sub,
                    left: Box::new(sig_len),
                    right: Box::new(script_size_expr),
                },
                span::Span::default(),
            )),
    ...
    let end = Expr::new(
        ExprKind::Binary {
            ...
            right: Box::new(Expr::int(field_payload_len as i64)),
        },
        span::Span::default(),
    );
    let substr = Expr::call(
        "OpTxInputScriptSigSubstr",
        vec![input_idx.clone(), start, end],
    );
...
pub(super) fn read_input_state_with_template_values<'i>(
    ...
    ): Result<&[Expr<'i>; 4], _> = args.try_into()
    else {
    ...
    if layout_field_types.is_empty() {
        return Err(CompilerError::Unsupported(
            "readInputStateWithTemplate requires a struct type".to_string(),
        ));
    }
...
pub(super) fn compile_contract_impl<'i>(
    ...
    let mut constants: HashMap<String, Expr<'i>> = contract
        .constants
        .iter()
        .map(|constant| (constant.name.clone(), constant.expr.clone()))
        .collect();
    ...
    let covenant_lowered_contract =
        lower_covenant_declarations(&inferred_lowered_contract, &constants)?;
    let read_input_state_lowered_contract =
        lower_read_input_state_calls(&covenant_lowered_contract)?;
    let inline_lowered_contract =
        lower_inline_functions(&read_input_state_lowered_contract, &mut debug_recorder)?;
    let structs = build_struct_registry(&inline_lowered_contract)?;
    let validate_output_state_lowered_contract =
        lower_validate_output_state(&inline_lowered_contract, &structs)?;
    let struct_lowered_contract = lower_structs_contract(
        &validate_output_state_lowered_contract,
        &structs,
        &constants,
    )?;
    ...
    let lowered_contract = if options.record_debug_infos {
        for_lowered_contract
    } else {
        lower_local_aliases(&for_lowered_contract)?
    };
    let mut lowered_constants = flatten_constructor_args_env(
        &covenant_lowered_contract.params,
        constructor_args,
        &structs,
    )?;
    lowered_constants.extend(
        lowered_contract
            .constants
            .iter()
            .map(|constant| (constant.name.clone(), constant.expr.clone())),
    ...
    let entrypoint_functions: Vec<&FunctionAst<'i>> = lowered_contract
        .functions
        .iter()
    ...
    if entrypoint_functions.is_empty() {
        return Err(CompilerError::Unsupported(
            "contract has no entrypoint functions".to_string(),
        ));
    }
    ...
    for _ in 0..32 {
        debug_recorder.record_contract_scope(
            &inline_lowered_contract,
            constructor_args,
            &structs,
        )?;
    ...
    Err(CompilerError::Unsupported(
        "script size did not stabilize".to_string(),
    ))
}
...
fn compile_contract_script_iteration<'i>(
    ...
    let (_contract_fields, field_prolog_script) = compile_contract_fields(
    ...
    let state_layout = CompiledStateLayout {
        start: selector_prefix_len,
        len: field_prolog_script.len(),
    };
    ...
    let script = build_contract_script(
        debug_recorder,
        without_selector,
        &field_prolog_script,
...
fn build_contract_script(
    ...
    if without_selector {
        let (_name, entrypoint_script) = compiled_entrypoints.first().ok_or_else(|| {
            CompilerError::Unsupported("contract has no entrypoint functions".to_string())
        })?;
...
fn contract_uses_script_size<'i>(contract: &ContractAst<'i>) -> bool {
    if contract
        .constants
        .iter()
        .any(|constant| expr_uses_script_size(&constant.expr))
    {
        return true;
    }
    if contract
        .fields
        .iter()
        .any(|field| expr_uses_script_size(&field.expr))
    {
        return true;
    }
    contract
        .functions
        .iter()
        .any(|func| func.body.iter().any(statement_uses_script_size))
}
...
fn statement_uses_script_size(stmt: &Statement<'_>) -> bool {
    match stmt {
        Statement::VariableDefinition { expr, .. } => {
            expr.as_ref().is_some_and(expr_uses_script_size)
        }
        ...
        Statement::If {
            ...
            expr_uses_script_size(condition)
                ...
                || else_branch
                    .as_ref()
                    .is_some_and(|branch| branch.iter().any(statement_uses_script_size))
        }
        Statement::For {
            start,
            end,
...
fn expr_uses_script_size<'i>(expr: &Expr<'i>) -> bool {
    match &expr.kind {
        ...
        ExprKind::Binary { left, right, .. } => {
            expr_uses_script_size(left) || expr_uses_script_size(right)
        }
        ExprKind::Append { source, args, .. } => {
            expr_uses_script_size(source) || args.iter().any(expr_uses_script_size)
        }
        ExprKind::IfElse {
            condition,
            then_expr,
            ...
            expr_uses_script_size(condition)
                || expr_uses_script_size(then_expr)
                || expr_uses_script_size(else_expr)
        }
        ...
        ExprKind::Split { source, index, .. } => {
            expr_uses_script_size(source) || expr_uses_script_size(index)
        }
        ExprKind::Slice {
            source, start, end, ..
        } => {
            expr_uses_script_size(source)
                || expr_uses_script_size(start)
                || expr_uses_script_size(end)
        }
...
fn infer_expr_type_ref_for_comparison<'i>(
    ...
    if let ExprKind::Call { name, .. } = &expr.kind {
        let is_builtin_cast = matches!(
            name.as_str(),
            "int" | "bool" | "byte" | "string" | "pubkey" | "sig" | "datasig"
        ) || (name.contains('[')
            && parse_type_ref(name)
                .ok()
                .is_some_and(|type_ref| !matches!(type_ref.base, TypeBase::Custom(_))));
    ...
    values.iter().all(|value| match &value.kind {
        ExprKind::Identifier(name) => types
            ...
            .is_some_and(|value_type| {
                is_type_assignable_ref(&value_type, &element_type, constants)
            }),
    ...
    contract
        ...
        .map(|func| FunctionAbiEntry {
            ...
            inputs: func
                .params
                .iter()
                .map(|param| FunctionInputAbi {
                    name: param.name.clone(),
                    type_name: type_name_from_ref(&param.type_ref),
                })
                .collect(),
        })
        .collect()
...
fn array_size_with_constants_ref<'i>(
    type_ref: &TypeRef,
    constants: &HashMap<String, Expr<'i>>,
) -> Option<usize> {
    match type_ref.array_size()? {
...
fn fixed_type_size_ref(type_ref: &TypeRef) -> Option<i64> {
    if type_ref.is_array() {
        if let (Some(elem_type), Some(size)) =
            (array_element_type_ref(type_ref), array_size_ref(type_ref))
        {
...
fn fixed_type_size_with_constants_ref<'i>(
    type_ref: &TypeRef,
    constants: &HashMap<String, Expr<'i>>,
) -> Option<usize> {
    if !type_ref.is_array() {
    ...
    match stmt {
        ...
        Statement::If {
            ...
            then_branch.iter().any(contains_return)
                || else_branch
                    .as_ref()
                    .is_some_and(|branch| branch.iter().any(contains_return))
        }
...
fn collect_expr_identifier_uses<'i>(expr: &Expr<'i>, uses: &mut HashMap<String, usize>) {
    match &expr.kind {
        ...
        ExprKind::UnarySuffix { source, .. } | ExprKind::FieldAccess { source, .. } => {
            collect_expr_identifier_uses(source, uses)
        }
        ExprKind::Int(_)
        | ExprKind::DateLiteral(_)
...
fn has_explicit_array_size_ref(type_ref: &TypeRef) -> bool {
    !matches!(
        type_ref.array_size(),
        Some(ArrayDim::Dynamic | ArrayDim::Inferred) | None
    )
}
...
fn is_array_type_assignable_ref<'i>(
    actual: &TypeRef,
    expected: &TypeRef,
    constants: &HashMap<String, Expr<'i>>,
) -> bool {
    ...
    match (
        array_size_with_constants_ref(actual, constants),
        array_size_with_constants_ref(expected, constants),
    ) {
...
pub(super) fn is_type_assignable_ref<'i>(
    actual: &TypeRef,
    expected: &TypeRef,
    constants: &HashMap<String, Expr<'i>>,
) -> bool {
...
fn coerce_rhs_byte_literal_for_comparison<'i>(
    left_type: Option<&TypeRef>,
    right: &Expr<'i>,
) -> Expr<'i> {
    if left_type
        .is_some_and(|type_ref| matches!(type_ref.base, TypeBase::Byte) && !type_ref.is_array())
...
fn infer_fixed_array_type_from_initializer_ref<'i>(
    ...
    match &init.kind {
        ExprKind::Array(values) => {
            ...
            if array_literal_matches_type_with_env_ref(values, &inferred, types, constants) {
                Some(inferred)
            } else {
                None
            }
        ...
        ExprKind::Identifier(name) => {
            ...
            if !is_array_type_ref(&other_type)
                || array_element_type_ref(&other_type) != Some(element_type.clone())
            {
                return None;
            }
...
fn array_size_with_constants<'i>(
    type_name: &str,
    constants: &HashMap<String, Expr<'i>>,
) -> Option<usize> {
    let type_ref = parse_type_ref(type_name).ok()?;
...
fn infer_fixed_array_type_from_initializer<'i>(
    ...
    infer_fixed_array_type_from_initializer_ref(&declared_type, initializer, types, constants)
        .map(|t| type_name_from_ref(&t))
}

fn encode_fixed_size_value<'i>(
    value: &Expr<'i>,
    type_name: &str,
) -> Result<Vec<u8>, CompilerError> {
    match type_name {
        "int" => {
            let number = match &value.kind {
                ...
                _ => {
                    return Err(CompilerError::Unsupported(
                        "array literal element type mismatch".to_string(),
                    ));
                }
            };
            serialize_i64(number, Some(8usize))
                ...
                .map_err(|err| {
                    CompilerError::Unsupported(format!(
                        "failed to serialize int literal {}: {err}",
                        number
                    ))
                })
        }
        ...
        "pubkey" => {
            ...
            Ok(bytes_exprs
                .iter()
                .filter_map(|value| {
                    if let ExprKind::Byte(byte) = &value.kind {
                        Some(*byte)
                    } else {
                        None
                    }
        ...
        _ => {
            ...
            if let (Some(inner_type), Some(size)) =
                (array_element_type(type_name), array_size(type_name))
            {
                if inner_type == "byte" {
                    let Some(len) = byte_array_len(value) else {
                        return Err(CompilerError::Unsupported(
            ...
            if let ExprKind::Array(values) = &value.kind {
                let element_type = array_element_type(type_name).ok_or_else(|| {
                    CompilerError::Unsupported(
                        "array element type must have known size".to_string(),
                    )
                })?;
                let expected_len = array_size(type_name).ok_or_else(|| {
                    CompilerError::Unsupported("array literal element type mismatch".to_string())
                })?;
...
pub(super) fn encode_array_literal<'i>(
    values: &[Expr<'i>],
    type_name: &str,
) -> Result<Vec<u8>, CompilerError> {
    let element_type = array_element_type(type_name).ok_or_else(|| {
    ...
    let mut out = Vec::new();
    debug_assert!(
        fixed_type_size(&element_type).is_some(),
        "type_check must validate array element type has known size"
    );
    for value in values {
...
fn infer_fixed_array_literal_type<'i>(values: &[Expr<'i>]) -> Option<String> {
    ...
    if values.iter().skip(1).all(|value| {
        infer_fixed_type_from_literal_expr(value).as_deref() == Some(first_type.as_str())
    }) {
...
pub fn function_branch_index<'i>(
    contract: &ContractAst<'i>,
    function_name: &str,
...
fn compile_entrypoint_function<'i>(
    ...
    for constant in contract_constants {
        types.insert(
            constant.name.clone(),
            type_name_from_ref(&constant.type_ref),
        );
    }
    ...
    for (index, stmt) in function.body.iter().enumerate() {
        if let Statement::Return { exprs, .. } = stmt {
            debug_assert_eq!(
                index,
                body_len - 1,
                "type_check must validate return statements are last"
            );
            for expr in exprs {
    ...
    if return_count == 0 {
        for _ in 0..stack_bindings
            .len()
            .saturating_sub(initial_stack_binding_count)
        {
...
fn compile_statement<'i>(
    ...
    ctx.debug_recorder
        .begin_statement_at(stmt, statement_start, ctx.types, ctx.stack_bindings);
    ...
    let added_stack_locals = match stmt {
        ...
        Statement::If {
            ...
            else_branch,
            ..
        } => compile_if_statement(ctx, stmt, condition, then_branch, else_branch.as_deref())
            .map(|_| Vec::new()),
        Statement::For { .. } => {
        ...
        Statement::TupleAssignment {
            ...
            expr,
            ..
        } => compile_tuple_assignment_statement(
            ctx,
            left_type_ref,
            left_name,
            right_type_ref,
        ...
        Statement::FunctionCall { name, args, .. } => {
            compile_function_call_statement(ctx, name, args)
        }
    ...
    ctx.debug_recorder.finish_statement_at(
        stmt,
...
fn compile_variable_definition_statement<'i>(
    ...
    let effective_type_name = if has_inferred_array_size_ref(type_ref) {
        infer_fixed_array_type_from_initializer(&type_name, expr, ctx.types, ctx.contract_constants)
            .ok_or_else(|| {
                CompilerError::Unsupported(format!(
    ...
    if is_array {
        let initial = array_initializer_expr(
            expr,
            &effective_type_name,
            ctx.types,
    ...
    let expr = expr.cloned().ok_or_else(|| {
...
fn compile_require_statement<'i>(
    ctx: &mut CompileStatementContext<'_, 'i>,
    expr: &Expr<'i>,
) -> Result<Vec<String>, CompilerError> {
    let mut stack_depth = 0i64;
...
fn compile_tuple_assignment_statement<'i>(
    ...
    match &expr.kind {
        ExprKind::Split {
            ...
            let mut added = compile_stack_variable_definition(
                ctx,
                left_name,
                type_name_from_ref(left_type_ref),
                left_expr,
            )?;
            added.extend(compile_stack_variable_definition(
                ctx,
                right_name,
                type_name_from_ref(right_type_ref),
                right_expr,
            )?);
            Ok(added)
        }
...
fn compile_assign_statement<'i>(
    ...
    if let Some(type_name) = ctx.types.get(name) {
        ...
        ctx.stack_bindings
            .emit_update_stack_for_rebinding(name, ctx.builder)?;
        return Ok(Vec::new());
    }
...
fn encoded_state_len<'i>(
    ...
    contract_fields.iter().try_fold(0usize, |acc, field| {
        Ok(acc + encoded_field_chunk_size(field, contract_constants)?)
    })
}
...
fn encoded_state_len_for_layout_field_types<'i>(
    ...
    layout_field_types.iter().try_fold(0usize, |acc, type_ref| {
        Ok(acc + encoded_field_chunk_size_for_type_ref(type_ref, contract_constants)?)
    })
}
...
fn templated_input_script_size_expr<'i>(
    ...
    let total_state_len =
        encoded_state_len_for_layout_field_types(layout_field_types, contract_constants)?;
    Ok(binary_expr(
        BinaryOp::Add,
        binary_expr(
            BinaryOp::Add,
            template_prefix_len.clone(),
            Expr::int(total_state_len as i64),
        ),
        ...
        ExprKind::Binary {
            ...
            left: Box::new(Expr::new(
                ExprKind::Binary {
                    op: BinaryOp::Sub,
                    left: Box::new(sig_len),
                    right: Box::new(Expr::int(script_size_value)),
                },
                span::Span::default(),
            )),
            right: Box::new(Expr::int(field_payload_offset as i64)),
...
fn read_input_state_field_expr_with_type<'i>(
    ...
    let field_payload_len =
        fixed_state_field_payload_len_for_type_ref(field_type, contract_constants).map_err(
            |_| {
                CompilerError::Unsupported(format!(
                    "{builtin_name} does not support field type {}",
                    type_name_from_ref(field_type)
                ))
            },
        )?;
    ...
    let start = binary_expr(
        BinaryOp::Add,
        input_sigscript_base_expr(input_idx, script_size_expr),
        field_payload_offset,
    );
    let end = binary_expr(
        BinaryOp::Add,
        start.clone(),
        Expr::int(field_payload_len as i64),
    );
...
fn cast_read_input_state_expr<'i>(
    substr: Expr<'i>,
    type_ref: &TypeRef,
) -> Result<Expr<'i>, CompilerError> {
    let type_name = type_name_from_ref(type_ref);
    match type_ref.base {
        TypeBase::Custom(_) => Err(CompilerError::Unsupported(format!(
            "readInputState does not support field type {type_name}"
        ))),
...
fn compile_read_input_state_statement<'i>(
    ...
    for binding in bindings {
        if bindings_by_field
            .insert(binding.field_name.as_str(), binding)
            .is_some()
        {
            return Err(CompilerError::Unsupported(format!(
    ...
    match name {
        "readInputState" => {
            if args.len() != 1 {
                return Err(CompilerError::Unsupported(
                    "readInputState(input_idx) expects 1 argument".to_string(),
                ));
            }
            if contract_fields.is_empty() {
                return Err(CompilerError::Unsupported(
                    "readInputState requires contract fields".to_string(),
                ));
            }
            if bindings_by_field.len() != contract_fields.len() {
                return Err(CompilerError::Unsupported(
                    "readInputState bindings must include all contract fields exactly once"
                        .to_string(),
                ));
            }

            let script_size_value = ctx.script_size.ok_or_else(|| {
                CompilerError::Unsupported("readInputState requires this.scriptSize".to_string())
            })?;
            ...
            let state_start_offset = contract_field_prefix_len
                .checked_sub(total_state_len)
                .ok_or_else(|| {
                    CompilerError::Unsupported("readInputState state offset underflow".to_string())
                })?;
        ...
        "readInputStateWithTemplate" => {
            ...
            let struct_spec = structs.get(&struct_name).ok_or_else(|| {
                CompilerError::Unsupported(format!("unknown struct '{struct_name}'"))
            })?;
            ...
            let layout_field_types = flattened_struct_field_specs_for_type(
                &TypeRef {
                    base: TypeBase::Custom(struct_name.clone()),
                    array_dims: Vec::new(),
                },
                structs,
            )?;
            ...
            for field in &struct_spec.fields {
                ...
                field_chunk_offset +=
                    encoded_field_chunk_size_for_type_ref(&field.type_ref, ctx.contract_constants)?;
            }
...
fn compile_read_input_state_with_template_validation(
    ...
    ): Result<&[Expr<'_>; 4], _> = args.try_into()
    else {
        return Err(CompilerError::Unsupported(
            "readInputStateWithTemplate(input_idx, template_prefix_len, template_suffix_len, expected_template_hash) expects 4 arguments"
                .to_string(),
    ...
    let prefix_end_expr = binary_expr(
        BinaryOp::Add,
        script_base_expr.clone(),
        prefix_len_expr.clone(),
    );
    let script_end_expr = binary_expr(
        BinaryOp::Add,
        script_base_expr.clone(),
        script_size_expr.clone(),
    );
    let state_len =
        encoded_state_len_for_layout_field_types(layout_field_types, contract_constants)?;
    let suffix_start_expr = binary_expr(
        BinaryOp::Add,
        prefix_end_expr.clone(),
        Expr::int(state_len as i64),
    );
    let suffix_end_expr = binary_expr(
        BinaryOp::Add,
        suffix_start_expr.clone(),
        suffix_len_expr.clone(),
    );

    let actual_redeem_script_expr =
        input_sigscript_substr_expr(input_idx, script_base_expr.clone(), script_end_expr);
    let actual_prefix_expr =
        input_sigscript_substr_expr(input_idx, script_base_expr, prefix_end_expr);
    let actual_suffix_expr =
        input_sigscript_substr_expr(input_idx, suffix_start_expr, suffix_end_expr);
...
fn compile_validate_output_state_inner_statement(
    ...
    let output_idx = args.first().ok_or_else(|| {
        CompilerError::Unsupported(
            "validateOutputState(output_idx, new_state) expects 2 arguments".to_string(),
        )
    })?;
    ...
    if contract_fields.is_empty() {
        return Err(CompilerError::Unsupported(
            "validateOutputState requires contract fields".to_string(),
        ));
    }
    ...
    let script_size_value = script_size.ok_or_else(|| {
        CompilerError::Unsupported("validateOutputState requires this.scriptSize".to_string())
    })?;
...
fn compile_validate_output_state_with_template_inner_statement(
    ...
    if state_fields.is_empty() {
        return Err(CompilerError::Unsupported(
            "validateOutputStateWithTemplate requires contract fields".to_string(),
        ));
    }

    let mut stack_depth = 0i64;

    let encoded_prefix_len = Expr::call(
        "bytes",
        vec![
            Expr::call("length", vec![template_prefix.clone()]),
            Expr::int(8),
        ],
    );
    let encoded_suffix_len = Expr::call(
        "bytes",
        vec![
            Expr::call("length", vec![template_suffix.clone()]),
            Expr::int(8),
        ],
    );
...
fn compile_encoded_state_object(
    ...
    for entry in state_entries {
        ...
        let type_name =
            infer_debug_expr_value_type(new_value, constants, types, &mut HashSet::new())?;
        let type_ref = parse_type_ref(&type_name)?;
        let field_size = fixed_state_field_payload_len_for_type_ref(&type_ref, contract_constants)
            .map_err(|_| {
                CompilerError::Unsupported(format!(
                    "{builtin_name} does not support field type {}",
                    type_name_from_ref(&type_ref)
...
fn compile_if_statement<'i>(
    ...
    ctx.debug_recorder.record_current_statement_source_step_at(
        stmt,
        ctx.builder.script().len(),
        ctx.types,
        ctx.stack_bindings,
    ...
    compile_block(
        &mut ctx.with_types_and_stack_bindings(&mut then_types, &mut then_stack_bindings),
        then_branch,
        true,
    )?;
    ...
    if let Some(else_branch) = else_branch {
        ...
        compile_block(
            &mut ctx.with_types_and_stack_bindings(&mut else_types, &mut else_stack_bindings),
            else_branch,
            true,
        )?;
...
fn compile_block<'i>(
    ...
    if scoped_stack_locals && !added_stack_locals.is_empty() {
        ctx.stack_bindings
            .emit_drop_bindings(&added_stack_locals, ctx.builder)?;
    ...
    match &expr.kind {
        ...
        ExprKind::Unary {
            ...
            value.checked_neg().ok_or_else(|| {
                CompilerError::InvalidLiteral(format!("constant integer overflow: -({value})"))
            })
        }
        ExprKind::Unary { .. } => Err(CompilerError::Unsupported(
            "for loop bounds must be constant integers".to_string(),
        )),
        ExprKind::Binary { op, left, right } => {
            ...
            match op {
                BinaryOp::Add => lhs.checked_add(rhs).ok_or_else(|| {
                    CompilerError::InvalidLiteral(format!(
                        "constant integer overflow: {lhs} + {rhs}"
                    ))
                }),
                ...
                BinaryOp::Mul => lhs.checked_mul(rhs).ok_or_else(|| {
                    CompilerError::InvalidLiteral(format!(
                        "constant integer overflow: {lhs} * {rhs}"
                    ))
                }),
...
pub(crate) fn resolve_expr<'i>(
    ...
    match kind {
        ...
        ExprKind::Unary { op, expr } => Ok(Expr::new(
            ExprKind::Unary {
                op,
                expr: Box::new(resolve_expr(*expr, constants, visiting)?),
            },
            span,
        )),
        ...
        ExprKind::Append {
            ...
            ExprKind::Append {
                ...
                args: args
                    .into_iter()
                    .map(|arg| resolve_expr(arg, constants, visiting))
...
pub(super) fn compile_expr<'i>(
    ...
    let mut ctx = CompileExprContext {
        scope,
        builder,
        options,
        visiting,
    ...
    match &expr.kind {
        ...
        ExprKind::IfElse {
            ...
            then_expr,
            else_expr,
        } => compile_if_else_expr(&mut ctx, condition, then_expr, else_expr),
...
fn compile_array_expr<'i>(
    ...
    let inferred_type =
        infer_fixed_array_runtime_type(values, ctx.scope.constants, ctx.scope.types).ok_or_else(
            || CompilerError::Unsupported("array literal type cannot be inferred".to_string()),
        )?;
...
fn infer_fixed_array_runtime_type<'i>(
    ...
    infer_fixed_array_literal_type(values).or_else(|| {
        let first_type =
            infer_debug_expr_value_type(values.first()?, constants, types, &mut HashSet::new())
                .ok()?;
        fixed_type_size(&first_type)?;
        if values.iter().skip(1).all(|value| {
            infer_debug_expr_value_type(value, constants, types, &mut HashSet::new())
                .ok()
                .as_deref()
                == Some(first_type.as_str())
        }) {
            Some(format!("{first_type}[]"))
        } else {
            None
...
fn compile_state_object_expr() -> Result<(), CompilerError> {
    Err(CompilerError::Unsupported(
        "state object literals are only supported in validateOutputState-style builtins"
            .to_string(),
    ))
}
...
fn compile_field_access_expr() -> Result<(), CompilerError> {
    Err(CompilerError::Unsupported(
        "struct field access should be lowered before compilation".to_string(),
    ))
}

fn compile_string_expr<'i>(
    ctx: &mut CompileExprContext<'_, 'i>,
    value: &str,
) -> Result<(), CompilerError> {
...
fn compile_identifier_expr<'i>(
    ...
    if ctx
        .scope
        .stack_bindings
        .emit_copy_binding_to_top(name, ctx.stack_depth, ctx.builder)?
    {
...
fn compile_new_expr<'i>(
    ...
    match name {
        ...
        "ScriptPubKeyP2PK" => {
            if args.len() != 1 {
                return Err(CompilerError::Unsupported(
                    "ScriptPubKeyP2PK expects a single pubkey argument".to_string(),
                ));
            }
            compile_expr_with_context(ctx, &args[0])?;
            ctx.builder
                .add_data_with_push_opcode(&[0x00, 0x00, OpData32])?;
            *ctx.stack_depth += 1;
        ...
        "ScriptPubKeyP2SH" => {
            if args.len() != 1 {
                return Err(CompilerError::Unsupported(
                    "ScriptPubKeyP2SH expects a single bytes32 argument".to_string(),
                ));
            }
            compile_expr_with_context(ctx, &args[0])?;
        ...
        "ScriptPubKeyP2SHFromRedeemScript" => {
            if args.len() != 1 {
                return Err(CompilerError::Unsupported(
                    "ScriptPubKeyP2SHFromRedeemScript expects a single redeem_script argument"
                        .to_string(),
                ));
            }
            compile_expr_with_context(ctx, &args[0])?;
...
fn compile_binary_expr<'i>(
    ...
    let left_cmp_type =
        infer_expr_type_ref_for_comparison(left, ctx.scope.constants, ctx.scope.types);
    let coerced_right = if matches!(
        op,
    ...
    let left_value_type = infer_debug_expr_value_type(
        left,
        ctx.scope.constants,
        ctx.scope.types,
        &mut HashSet::new(),
    ...
    .ok();
    let right_value_type = infer_debug_expr_value_type(
        &coerced_right,
        ctx.scope.constants,
        ctx.scope.types,
        &mut HashSet::new(),
    ...
    debug_assert!(
        !matches!(op, BinaryOp::Add)
            || (left_value_type.as_deref() != Some("byte")
                && right_value_type.as_deref() != Some("byte")),
    ...
    let bytes_add = matches!(op, BinaryOp::Add)
        && (expr_is_bytes(left, ctx.scope.types) || expr_is_bytes(right, ctx.scope.types));
    if bytes_add {
    ...
    match op {
        ...
        BinaryOp::Eq => {
            ctx.builder
                .add_op(if bytes_eq { OpEqual } else { OpNumEqual })?;
        }
        BinaryOp::Ne => {
            if bytes_eq {
...
fn compile_unary_suffix_expr<'i>(
    ...
    match kind {
        ...
        UnarySuffixKind::Reverse => Err(CompilerError::Unsupported(
            "reverse() is not supported".to_string(),
        )),
    }
}
...
fn compile_array_index_expr<'i>(
    ...
    let source_type = infer_debug_expr_value_type(
        &resolved_source,
        ctx.scope.constants,
        ctx.scope.types,
        &mut HashSet::new(),
    )?;
    let element_type = array_element_type(&source_type).ok_or_else(|| {
        CompilerError::Unsupported(format!(
            "array index requires array source, got {source_type}"
        ))
    })?;
    ...
    match op {
        ...
        NullaryOp::ThisScriptSize => {
            let size = ctx.script_size.ok_or_else(|| {
                CompilerError::Unsupported(
                    "this.scriptSize is only available at compile time".to_string(),
                )
            })?;
        ...
        NullaryOp::ThisScriptSizeDataPrefix => {
            let size = ctx.script_size.ok_or_else(|| {
                CompilerError::Unsupported(
                    "this.scriptSizeDataPrefix is only available at compile time".to_string(),
                )
            })?;
            let size: usize = size.try_into().map_err(|_| {
                CompilerError::Unsupported(
                    "this.scriptSizeDataPrefix requires a non-negative script size".to_string(),
                )
            })?;
...
fn compile_date_literal_expr<'i>(
    ctx: &mut CompileExprContext<'_, 'i>,
    value: i64,
) -> Result<(), CompilerError> {
    ctx.builder.add_i64(value)?;
...
fn compile_number_with_unit_expr() -> Result<(), CompilerError> {
    Err(CompilerError::Unsupported(
        "number units must be normalized during parsing".to_string(),
    ))
}
...
fn expr_is_bytes_inner<'i>(
    ...
    match &expr.kind {
        ...
        ExprKind::Binary {
            ...
            expr_is_bytes_inner(left, types, visiting)
                || expr_is_bytes_inner(right, types, visiting)
        }
        ExprKind::IfElse {
            condition: _,
            then_expr,
            ...
            expr_is_bytes_inner(then_expr, types, visiting)
                && expr_is_bytes_inner(else_expr, types, visiting)
        }
        ...
        ExprKind::Identifier(name) => {
            ...
            types
                .get(name)
                .map(|type_name| is_bytes_type(type_name))
                .unwrap_or(false)
        }
...
fn compile_call_expr<'i>(
    ...
    let mut ctx = CompileCallContext {
        scope,
        builder,
        options,
        visiting,
    ...
    match name {
        ...
        "OpTxPayloadSubstr" => {
            compile_opcode_builtin_call(&mut ctx, name, args, 2, OpTxPayloadSubstr)
        }
        ...
        "OpTxInputScriptSigLen" => {
            compile_opcode_builtin_call(&mut ctx, name, args, 1, OpTxInputScriptSigLen)
        }
        "OpTxInputScriptSigSubstr" => {
            compile_opcode_builtin_call(&mut ctx, name, args, 3, OpTxInputScriptSigSubstr)
        }
        "OpTxInputSeq" => compile_opcode_builtin_call(&mut ctx, name, args, 1, OpTxInputSeq),
        "OpTxInputDaaScore" => {
            compile_opcode_builtin_call(&mut ctx, name, args, 1, OpTxInputDaaScore)
        }
        "OpTxInputIsCoinbase" => {
            compile_opcode_builtin_call(&mut ctx, name, args, 1, OpTxInputIsCoinbase)
        }
        "OpTxInputSpkLen" => compile_opcode_builtin_call(&mut ctx, name, args, 1, OpTxInputSpkLen),
        "OpTxInputSpkSubstr" => {
            compile_opcode_builtin_call(&mut ctx, name, args, 3, OpTxInputSpkSubstr)
        }
        "OpTxOutputSpkLen" => {
            compile_opcode_builtin_call(&mut ctx, name, args, 1, OpTxOutputSpkLen)
        }
        "OpTxOutputSpkSubstr" => {
            compile_opcode_builtin_call(&mut ctx, name, args, 3, OpTxOutputSpkSubstr)
        }
        "OpAuthOutputCount" => {
            compile_opcode_builtin_call(&mut ctx, name, args, 1, OpAuthOutputCount)
        }
        "OpAuthOutputIdx" => compile_opcode_builtin_call(&mut ctx, name, args, 2, OpAuthOutputIdx),
        "OpInputCovenantId" => {
            compile_opcode_builtin_call(&mut ctx, name, args, 1, OpInputCovenantId)
        }
        "OpOutputCovenantId" => {
            compile_opcode_builtin_call(&mut ctx, name, args, 1, OpOutputCovenantId)
        }
        ...
        "OpCovOutputCount" => {
            compile_opcode_builtin_call(&mut ctx, name, args, 1, OpCovOutputCount)
        }
        ...
        "OpChainblockSeqCommit" => {
            compile_opcode_builtin_call(&mut ctx, name, args, 1, OpChainblockSeqCommit)
        }
        "bytes" => compile_bytes_call(&mut ctx, args),
        "length" => compile_length_call(&mut ctx, args),
        "int" | "byte" | "bool" | "string" | "sig" | "pubkey" | "datasig" => {
            compile_passthrough_cast_call(&mut ctx, name, args)
        }
        ...
        "checkSigFromStack" => {
            compile_checksigfromstack_call(&mut ctx, name, args, OpCheckSigFromStack)
        }
        "checkSigFromStackECDSA" => {
            compile_checksigfromstack_call(&mut ctx, name, args, OpCheckSigFromStackECDSA)
        }
...
fn compile_call_arg_with_context<'i>(
...
fn compile_sha256_call<'i>(
    ctx: &mut CompileCallContext<'_, 'i>,
    args: &[Expr<'i>],
) -> Result<(), CompilerError> {
    if args.len() != 1 {
        return Err(CompilerError::Unsupported(
            "sha256() expects a single argument".to_string(),
        ));
    }
...
fn compile_bytes_call<'i>(
    ctx: &mut CompileCallContext<'_, 'i>,
    args: &[Expr<'i>],
) -> Result<(), CompilerError> {
    if args.is_empty() || args.len() > 2 {
        return Err(CompilerError::Unsupported(
            "bytes() expects one or two arguments".to_string(),
        ));
    }
    if args.len() == 2 {
        compile_call_arg_with_context(ctx, &args[0])?;
...
fn compile_blake2b_with_key_call<'i>(
    ctx: &mut CompileCallContext<'_, 'i>,
    args: &[Expr<'i>],
) -> Result<(), CompilerError> {
    let Ok([data, key]): Result<&[Expr<'i>; 2], _> = args.try_into() else {
        return Err(CompilerError::Unsupported(
            "blake2bWithKey() expects 2 arguments".to_string(),
        ));
    };
...
fn compile_blake3_call<'i>(
    ctx: &mut CompileCallContext<'_, 'i>,
    args: &[Expr<'i>],
) -> Result<(), CompilerError> {
    if args.len() != 1 {
        return Err(CompilerError::Unsupported(
            "blake3() expects a single argument".to_string(),
        ));
    }
    compile_call_arg_with_context(ctx, &args[0])?;
...
fn compile_blake3_with_key_call<'i>(
    ctx: &mut CompileCallContext<'_, 'i>,
    args: &[Expr<'i>],
) -> Result<(), CompilerError> {
    let Ok([data, key]): Result<&[Expr<'i>; 2], _> = args.try_into() else {
        return Err(CompilerError::Unsupported(
            "blake3WithKey() expects 2 arguments".to_string(),
        ));
    };
    compile_call_arg_with_context(ctx, data)?;
    compile_call_arg_with_context(ctx, key)?;
...
fn compile_template_hash_call<'i>(
    ctx: &mut CompileCallContext<'_, 'i>,
    args: &[Expr<'i>],
) -> Result<(), CompilerError> {
    let Ok([prefix, suffix]): Result<&[Expr<'i>; 2], _> = args.try_into() else {
        return Err(CompilerError::Unsupported(
            "templateHash() expects 2 arguments".to_string(),
        ));
    };

    let encoded_prefix_len = Expr::call(
        "bytes",
        vec![Expr::call("length", vec![prefix.clone()]), Expr::int(8)],
    );
    let encoded_suffix_len = Expr::call(
        "bytes",
        vec![Expr::call("length", vec![suffix.clone()]), Expr::int(8)],
    );
...
fn compile_checksigfromstack_call<'i>(
    ...
    if args.len() != 3 {
        return Err(CompilerError::Unsupported(format!(
            "{name}() expects 3 arguments (signature, digest, publicKey)"
        )));
    }
    compile_call_arg_with_context(ctx, &args[0])?;
    compile_call_arg_with_context(ctx, &args[1])?;
...
pub(crate) fn is_bytes_type(type_name: &str) -> bool {
    if type_name == "bytes"
        || type_name == "byte"
        || matches!(type_name, "pubkey" | "sig" | "datasig" | "string")
    {
        return true;
    }
        ...
        match &item.kind {
            ...
            ExprKind::Call { name, args, .. } if name == "bytes" || name == "byte[]" => {
                ...
                match &args[0].kind {
                    ...
                    _ => {
                        return Err(CompilerError::Unsupported(
                            "byte[]() in LockingBytecodeNullData only supports string literals"
                                .to_string(),
                        ));
                    }
                }
...
mod tests {
    ...
    fn entrypoint_stack_setup_places_contract_fields_above_params_in_depth_order() {
        ...
        assert_eq!(
            ...
            ["field_b", "field_a", "param_b", "param_a"]
                .into_iter()
                .map(str::to_string)
                .collect::<Vec<_>>()
        );
