...
use crate::ast::{
    ContractAst, ContractFieldAst, Expr, ExprKind, FunctionAst, ParamAst, StateFieldExpr,
    Statement, StructBindingAst, TypeRef,
};
use crate::debug_info::{
    DebugFunctionRange, DebugInfo, DebugInfoRecorder, DebugLeafBinding, DebugNamedValue,
    DebugParamBinding, DebugParamMapping, DebugStackBinding, DebugStep, DebugVariableUpdate,
    SourceSpan, StepKind,
};

use super::stack_bindings::StackBindings;
use super::{
    CompileOptions, CompilerError, StructRegistry, build_struct_registry, flatten_type_ref_leaves,
    is_struct, is_struct_array, type_name_from_ref,
};
...
impl<'i> DebugRecorder<'i> {
    ...
    pub(crate) fn record_contract_scope(
        ...
        for function in &contract.functions {
            ...
            let structured_leaf_specs =
                build_structured_leaf_specs_for_function(function, structs, visible_names)?;
            active
                .structured_leaf_specs_by_function
                .insert(function.name.clone(), structured_leaf_specs);
        }
    ...
    pub(super) fn finish_source_function(&mut self) {
        ...
        debug_assert!(
            active.active_source_inline_frames.is_empty(),
            "source function ended with unclosed inline frames"
        );
    ...
    pub(super) fn current_source_statement_index(&self) -> usize {
        self.active
            .as_ref()
            .map_or(0, |active| active.current_source_statement_index)
    }
    ...
    pub(super) fn begin_inline_source_call(&mut self, callee: &str, call_site_span: SourceSpan) {
        ...
        let frame_depth = active
            .active_source_inline_frames
            .last()
            .map(|frame| frame.frame_depth.saturating_add(1))
            .unwrap_or(1);
    ...
    pub(super) fn record_inline_source_param(
        &mut self,
        name: &str,
        type_ref: &TypeRef,
        expr: Expr<'i>,
        ...
        let rewritten_expr = active
            ...
            .as_deref()
            .map(|function_name| {
                rewrite_debug_expr_with_function(
                    expr.clone(),
                    function_name,
                    &active.visible_names_by_function,
                )
            })
            .unwrap_or(expr);
    ...
    pub(super) fn finish_inline_source_call(&mut self, body_end_statement_index: usize) {
        ...
        active
            .inline_frame_plans_by_function
            .entry(function_name.to_string())
            .or_default()
            .push(InlineFramePlan {
                callee: frame.callee,
    ...
    pub(crate) fn visible_name(&self, lowered_name: &str) -> String {
        ...
        if let Some(spec) = active
            .structured_leaf_specs_by_function
            .get(function_name)
            .and_then(|specs| specs.get(lowered_name))
        {
    ...
    pub(crate) fn rewrite_debug_expr(&self, expr: Expr<'i>) -> Expr<'i> {
        ...
        let kind = match expr.kind {
            ...
            ExprKind::Array(values) => ExprKind::Array(
                values
                    .into_iter()
                    .map(|value| self.rewrite_debug_expr(value))
                    .collect(),
            ),
            ExprKind::Call {
                name,
                ...
                args: args
                    .into_iter()
                    .map(|arg| self.rewrite_debug_expr(arg))
                    .collect(),
                name_span,
            },
            ExprKind::New {
            ...
            ExprKind::Unary { op, expr } => ExprKind::Unary {
                op,
                expr: Box::new(self.rewrite_debug_expr(*expr)),
            },
    ...
    pub(crate) fn begin_entrypoint(
        ...
        debug_assert!(
            active.active_entrypoint.is_none(),
            "begin_entrypoint called while another entrypoint is active"
        );
        if !active
            .structured_leaf_specs_by_function
    ...
    pub(crate) fn record_current_statement_source_step_at(
        ...
        let current_updates = collect_variable_updates(
            self,
            stmt,
            &HashMap::new(),
            &StackBindings::default(),
            after_types,
        ...
        let Some((
            ...
            already_recorded,
            accumulated_updates,
            accumulated_console_args,
        )) = self
            .active
            .as_ref()
            .and_then(|active| active.statement_debug_state_stack.last())
            .map(|state| {
                (
    ...
    let kind = match expr.kind {
        ...
        ExprKind::Call {
            ...
            args: args
                .into_iter()
                .map(|arg| {
                    rewrite_debug_expr_with_function(arg, function_name, visible_names_by_function)
                })
                .collect(),
            name_span,
        },
        ...
        ExprKind::Split {
            ...
            source: Box::new(rewrite_debug_expr_with_function(
                *source,
                function_name,
                visible_names_by_function,
            )),
            index: Box::new(rewrite_debug_expr_with_function(
                *index,
                function_name,
                visible_names_by_function,
            )),
        ...
        ExprKind::Slice {
            ...
            start: Box::new(rewrite_debug_expr_with_function(
                *start,
                function_name,
                visible_names_by_function,
            )),
            end: Box::new(rewrite_debug_expr_with_function(
                *end,
                function_name,
                visible_names_by_function,
            )),
        ...
        ExprKind::Unary { op, expr } => ExprKind::Unary {
            op,
            expr: Box::new(rewrite_debug_expr_with_function(
                *expr,
                function_name,
                visible_names_by_function,
            )),
        ...
        ExprKind::Binary { op, left, right } => ExprKind::Binary {
            op,
            left: Box::new(rewrite_debug_expr_with_function(
                *left,
                function_name,
                visible_names_by_function,
            )),
            right: Box::new(rewrite_debug_expr_with_function(
                *right,
                function_name,
                visible_names_by_function,
            )),
        ...
        ExprKind::IfElse {
            ...
            condition: Box::new(rewrite_debug_expr_with_function(
                *condition,
                function_name,
                visible_names_by_function,
            )),
            then_expr: Box::new(rewrite_debug_expr_with_function(
                *then_expr,
                function_name,
                visible_names_by_function,
            )),
            else_expr: Box::new(rewrite_debug_expr_with_function(
                *else_expr,
                function_name,
                visible_names_by_function,
            )),
        ...
        ExprKind::StructLiteral(fields) => ExprKind::StructLiteral(
            fields
                ...
                .map(|field| StateFieldExpr {
                    name: field.name,
                    expr: rewrite_debug_expr_with_function(
                        field.expr,
                        function_name,
                        visible_names_by_function,
                    ),
...
fn collect_console_args<'i>(recorder: &DebugRecorder<'i>, stmt: &Statement<'i>) -> Vec<Expr<'i>> {
    match stmt {
        Statement::Console { args, .. } => args
            .iter()
            .cloned()
            .map(|expr| recorder.rewrite_debug_expr(expr))
            .collect(),
        _ => Vec::new(),
    }
}
...
fn collect_variable_updates<'i>(
    ...
    match stmt {
        ...
        .flatten()
        .collect(),
        Statement::Assign { name, expr, .. } => vec![build_runtime_debug_update(
            recorder,
            name,
            expr.clone(),
            after_types,
        ...
        | Statement::StructDestructure { bindings, .. } => bindings
            .iter()
            .filter_map(|binding| {
                build_runtime_debug_update(
                    recorder,
                    &binding.name,
                    Expr::new(
                        ExprKind::Identifier(binding.name.clone()),
                        crate::span::Span::default(),
                    ),
                    after_types,
                    after_stack_bindings,
...
fn merge_debug_variable_updates<'i>(
    target: &mut Vec<DebugVariableUpdate<'i>>,
    updates: Vec<DebugVariableUpdate<'i>>,
) {
    for update in updates {
        if let Some(existing) = target
            .iter_mut()
            .find(|existing| existing.name == update.name)
        {
            ...
            match (
                &mut existing.structured_leaf_bindings,
                update.structured_leaf_bindings,
            ) {
                (Some(existing_leaves), Some(update_leaves)) => {
                    for leaf in update_leaves {
                        if let Some(existing_leaf) = existing_leaves
                            .iter_mut()
                            .find(|existing_leaf| existing_leaf.field_path == leaf.field_path)
                        {
                            *existing_leaf = leaf;
                        } else {
...
fn build_runtime_debug_update<'i>(
    ...
    if let Some(update) = build_structured_root_debug_update(
        recorder,
        lowered_name,
        expr.clone(),
        after_stack_bindings,
    ...
    let stack_binding = after_stack_bindings.depth(lowered_name).map(|from_top| {
        crate::debug_info::DebugStackBinding {
            from_top,
            stack_height: Some(after_stack_bindings.len()),
        }
    });
    if let Some(spec) = recorder
        ...
        .and_then(|function_name| {
            active_structured_leaf_spec(recorder, function_name, lowered_name)
        })
    {
...
fn build_structured_root_debug_update<'i>(
    ...
    let mut leaf_bindings = specs
        ...
        .map(|(leaf_name, spec)| DebugLeafBinding {
            field_path: spec.field_path.clone(),
            type_name: spec.leaf_type_name.clone(),
            stack_binding: after_stack_bindings.depth(leaf_name).map(|from_top| {
                DebugStackBinding {
                    from_top,
                    stack_height: Some(after_stack_bindings.len()),
                }
    ...
    let first_spec = specs
        .iter()
        .find(|(leaf_name, _)| leaf_name.starts_with(&fallback_prefix))
        .map(|(_, spec)| spec)?;
...
impl<'i> ActiveDebugRecorder<'i> {
    ...
    fn start_statement_slot(
        &mut self,
        stmt: &Statement<'i>,
        bytecode_start: usize,
    ) -> StatementDebugState<'i> {
        let span = SourceSpan::from(stmt.span());
        let mut source_path = self
            .statement_debug_state_stack
            .iter()
            .map(|state| state.span)
            .collect::<Vec<_>>();
        source_path.push(span);
        let reuses_ancestor_source_slot = self
            .statement_debug_state_stack
            .iter()
            .any(|state| state.continued_across_siblings);
    ...
    fn take_pending_statement_slot(&mut self, depth: usize) -> Option<StatementDebugState<'i>> {
        self.pending_statement_debug_states
            .get_mut(depth)
    ...
    fn store_pending_statement_slot(&mut self, depth: usize, state: StatementDebugState<'i>) {
        if self.pending_statement_debug_states.len() <= depth {
            self.pending_statement_debug_states
                .resize_with(depth + 1, || None);
        }
        debug_assert!(
            self.pending_statement_debug_states[depth].is_none(),
            "pending source slot already present at depth {depth}"
        );
...
impl<'i> StagedEntrypointDebug<'i> {
    ...
    fn new_statement_index_for_path(&mut self, source_path: Vec<SourceSpan>) -> usize {
        ...
        self.statement_index_by_path
            .insert(source_path, statement_index);
        statement_index
    }
    ...
    fn emit_inline_call_enters(&mut self, statement_index: usize, bytecode_start: usize) {
        let mut plans = self
            .frame_plans
            .iter()
            .filter(|plan| plan.start_statement_index == statement_index)
            .cloned()
            .collect::<Vec<_>>();
    ...
    fn emit_inline_call_exits(&mut self, statement_end_index: usize, bytecode_end: usize) {
        let mut plans = self
            .frame_plans
            .iter()
            .filter(|plan| plan.body_end_statement_index == statement_end_index)
            .cloned()
            .collect::<Vec<_>>();
        ...
        for plan in plans {
            ...
            let variable_updates = self
                ...
                .rev()
                .find(|step| {
                    matches!(step.kind, StepKind::Source {}) && step.frame_id == plan.frame_id
                })
    ...
    fn emit_inline_call_resumes(&mut self, statement_index: usize, bytecode_start: usize) {
        let mut plans = self
            .frame_plans
            .iter()
            .filter(|plan| plan.resume_statement_index == statement_index)
            .cloned()
            .collect::<Vec<_>>();
        plans.sort_by_key(|plan| std::cmp::Reverse(plan.frame_depth));
    ...
    fn current_frame_context(&self, statement_index: usize) -> (u32, u32) {
        self.frame_plans
            .iter()
            .filter(|plan| {
                plan.start_statement_index <= statement_index
                    && statement_index < plan.body_end_statement_index
            })
...
fn build_structured_leaf_specs_for_function<'i>(
    ...
    for param in &function.params {
        record_structured_binding_spec(
            &mut specs,
            &param.name,
            &param.type_ref,
            visible_names,
    ...
    collect_structured_binding_specs_from_statements(
        &mut specs,
        &function.body,
        visible_names,
        structs,
...
fn inline_param_leaf_bindings(
    type_ref: &TypeRef,
    structs: &StructRegistry,
) -> Option<Vec<DebugLeafBinding>> {
...
fn source_statement_slot_count(statement: &Statement<'_>) -> usize {
    match statement {
        Statement::Block { body, .. } => {
            1 + body.iter().map(source_statement_slot_count).sum::<usize>()
        }
        Statement::If {
            then_branch,
            else_branch,
            ...
            1 + then_branch
                .iter()
                .map(source_statement_slot_count)
                .sum::<usize>()
                + else_branch
                    .as_ref()
                    .map(|branch| {
...
fn record_structured_binding_spec(
    ...
    let visible_base_name = visible_names
        .and_then(|names| names.get(lowered_base_name))
        .cloned()
        .unwrap_or_else(|| lowered_base_name.to_string());
...
fn build_param_mappings<'i>(
    ...
    for (param, spec) in params_source.iter().zip(param_specs) {
        let binding = match spec {
            ParamBindingSpec::Scalar => DebugParamBinding::SingleValue {
            ...
            ParamBindingSpec::Structured(leaf_specs) => DebugParamBinding::StructuredValue {
                leaf_bindings: leaf_specs
                    ...
                    .map(|(field_path, type_name)| DebugLeafBinding {
                        field_path,
                        type_name,
                        stack_binding: Some(DebugStackBinding {
                            from_top: next_stack_index(),
                            stack_height: None,
                        }),
                    })
                    .collect(),
    ...
    for (index, field) in contract_fields.iter().enumerate() {
        params.push(DebugParamMapping {
            ...
            binding: DebugParamBinding::SingleValue {
...
mod tests {
    ...
    fn disabled_recorder_returns_no_debug_info() {
        ...
        let mut recorder =
            DebugRecorder::new(CompileOptions::default(), &contract).expect("recorder");
        let structs = build_struct_registry(&contract).expect("structs");
        recorder
            .record_contract_scope(&contract, &[], &structs)
            .expect("record contract scope");
        assert!(
            recorder
                .take_debug_info(Some("contract Demo() {}"))
                .is_none()
        );
    }
    ...
    fn enabled_recorder_records_contract_scope() {
        ...
        let structs = build_struct_registry(&contract).expect("structs");
        recorder
            .record_contract_scope(&contract, &[Expr::int(7)], &structs)
            .expect("record contract scope");
        let debug_info = recorder
            .take_debug_info(Some("contract Demo(int seed) {}"))
            .expect("debug info");
    ...
    fn begin_entrypoint_keeps_bytecode_metadata_separate_from_source_steps() {
        ...
        recorder
            ...
            .expect("record contract scope");
        recorder
            .begin_entrypoint(function, &contract.fields, &structs)
            .expect("begin entrypoint");
