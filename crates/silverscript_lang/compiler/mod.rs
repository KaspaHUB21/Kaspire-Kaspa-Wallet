...
use crate::ast::{
    ArrayDim, BinaryOp, ConstantAst, ContractAst, ContractFieldAst, Expr, ExprKind, FunctionAst,
    IntrospectionKind, NullaryOp, ParamAst, STATE_TYPE_NAME, SplitPart, StateFieldExpr, Statement,
    StructBindingAst, TimeVar, TypeBase, TypeRef, UnaryOp, UnarySuffixKind, parse_contract_ast,
    parse_type_ref,
};
...
pub(super) use structs::{
    StructRegistry, build_struct_registry, ensure_known_or_builtin_type,
    flatten_constructor_args_env, flatten_type_ref_leaves, flattened_struct_field_specs_for_type,
    is_struct, is_struct_array, lower_runtime_expr, lower_runtime_struct_expr,
...
impl<'i> ContractAst<'i> {
    ...
    pub fn resolve_contract_state_values(
        &self,
        constructor_args: &[Expr<'i>],
        ...
        let mut env: HashMap<String, Expr<'i>> = self
            .constants
            .iter()
        ...
        for field in &self.fields {
            ...
            resolved_fields.push(DebugNamedValue {
...
impl<'i> CompiledContract<'i> {
    ...
    pub fn template_hash(&self) -> [u8; 32] {
        ...
        crate::template::template_hash(
            &self.script[..self.state_layout.start],
            &self.script[state_end..],
        )
    }
    ...
    pub fn build_sig_script(
        ...
        let function = self
            ...
            .ok_or_else(|| {
                CompilerError::Unsupported(format!("function '{}' not found", function_name))
            })?;
...
fn push_typed_sigscript_arg<'i>(
    ...
    if let Some(element_type) = type_ref.array_element_type() {
        if let Some(struct_name) = struct_name_from_type_ref(&element_type, structs) {
            ...
            let ExprKind::Array(values) = arg.kind else {
                return Err(CompilerError::Unsupported(
                    "signature script struct array arguments must be array literals".to_string(),
                ));
            };
            ...
            for field in &item.fields {
                ...
                for value in &values {
                    let ExprKind::StructLiteral(entries) = &value.kind else {
                        return Err(CompilerError::Unsupported(
                            "signature script struct array arguments must contain object literals"
                                .to_string(),
                        ));
                    };
                    ...
                    field_values.push(matched.ok_or_else(|| {
                        CompilerError::Unsupported(format!(
                    ...
                    if let Some(extra) = entries
                        .iter()
                        .find(|entry| item.fields.iter().all(|field| field.name != entry.name))
                    {
                        return Err(CompilerError::Unsupported(format!(
    ...
    if let Some(struct_name) = struct_name_from_type_ref(type_ref, structs) {
        ...
        let ExprKind::StructLiteral(fields) = arg.kind else {
            return Err(CompilerError::Unsupported(
                "signature script struct arguments must be object literals".to_string(),
            ));
        };
        ...
        for field in &item.fields {
            let value = provided.remove(&field.name).ok_or_else(|| {
                CompilerError::Unsupported(format!(
                    "struct field '{}' must be initialized",
                    field.name
                ))
    ...
    if !value_matches_type_ref(&arg, type_ref) {
        return Err(CompilerError::Unsupported(
            "signature script arguments must match the declared type".to_string(),
        ));
    }
    ...
    if compile::is_array_type(&type_name) {
        match &arg.kind {
            ...
            _ => Err(CompilerError::Unsupported(
                "signature script arguments must be literals".to_string(),
            )),
        }
    } else {
...
fn input_sigscript_substr_expr<'i>(
    input_idx: &Expr<'i>,
    start: Expr<'i>,
    end: Expr<'i>,
) -> Expr<'i> {
