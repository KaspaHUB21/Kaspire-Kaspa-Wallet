...
use crate::parser::{
    Rule, parse_expression as parse_expression_rule, parse_source_file,
...
impl<'i> Expr<'i> {
    ...
    pub fn bytes(value: Vec<u8>) -> Self {
        Self::new(
            ExprKind::Array(value.into_iter().map(Expr::byte).collect()),
            Span::default(),
        )
    }
    ...
    fn write_contract_item(&mut self, item: ContractItemRef<'_, '_>) {
        match item {
            ...
            ContractItemRef::Constant(constant) => {
                self.line(&format!(
                    "{} constant {} = {};",
                    constant.type_ref.type_name(),
                    constant.name,
                    format_expr(&constant.expr)
                ));
            }
    ...
    fn write_statement(&mut self, statement: &Statement<'_>) {
        match statement {
            ...
            Statement::TimeOp {
                ...
                let message = message
                    .as_ref()
                    .map(|message| format!(", {}", format_string_literal(message)))
                    .unwrap_or_default();
                self.line(&format!(
                    "require({} >= {}{});",
                    format_time_var(*tx_var),
                    format_expr(expr),
                    message
                ));
            }
...
fn ordered_contract_items<'a, 'i>(contract: &'a ContractAst<'i>) -> Vec<ContractItemRef<'a, 'i>> {
    let has_real_spans = contract.structs.iter().any(|item| !item.span.is_empty())
        ...
        || contract
            .constants
            .iter()
            .any(|constant| !constant.span.is_empty())
        || contract
            .functions
            .iter()
            .any(|function| !function.span.is_empty());

    let mut items = Vec::with_capacity(
        contract.structs.len()
            + contract.fields.len()
...
fn format_params(params: &[ParamAst<'_>]) -> String {
    params
        .iter()
        .map(|param| format!("{} {}", param.type_ref.type_name(), param.name))
        .collect::<Vec<_>>()
        .join(", ")
}
...
fn format_expr_with_prec(expr: &Expr<'_>, parent_prec: u8, right_child: bool) -> String {
    ...
    let rendered = match &expr.kind {
        ...
        ExprKind::Unary { op, expr } => format!(
            "{}{}",
            unary_op_str(*op),
            format_expr_with_prec(expr, PREC_UNARY, false)
        ),
        ...
        ExprKind::Introspection { kind, index, .. } => {
            format!(
                "{}[{}]{}",
                introspection_root(*kind),
                format_expr(index),
                introspection_field(*kind)
            )
        }
...
fn introspection_field(kind: IntrospectionKind) -> &'static str {
    match kind {
        ...
        IntrospectionKind::InputScriptPubKey | IntrospectionKind::OutputScriptPubKey => {
            ".scriptPubKey"
        }
...
pub fn parse_type_ref(type_name: &str) -> Result<TypeRef, CompilerError> {
    ...
    let pair = pairs
        .next()
        .ok_or_else(|| CompilerError::Unsupported("missing type name".to_string()))?;
...
fn parse_type_name_pair(pair: Pair<'_, Rule>) -> Result<TypeRef, CompilerError> {
    ...
    let base = match inner
        .next()
        .ok_or_else(|| CompilerError::Unsupported("missing base type".to_string()))?
        .as_str()
    {
        "int" => TypeBase::Int,
        "bool" => TypeBase::Bool,
            ...
            Some(size_pair) => match size_pair.as_rule() {
                ...
                _ => {
                    return Err(CompilerError::Unsupported(
                        "invalid array dimension".to_string(),
                    ));
                }
            },
        };
        ...
        match pair.as_rule() {
            Rule::pragma_directive => {
                if pragma.is_some() {
                    ...
                    return Err(CompilerError::Unsupported(
                        "multiple pragma directives are not supported".to_string(),
                    )
                    .with_span(&span));
                }
            ...
            Rule::contract_definition => {
                contract = Some(parse_contract_definition(pair, pragma.clone())?)
            }
            _ => {}
        }
    }
...
pub fn parse_expression_ast<'i>(source: &'i str) -> Result<Expr<'i>, CompilerError> {
    ...
    let expr_pair = pairs
        .next()
        .ok_or_else(|| CompilerError::Unsupported("empty expression".to_string()))?;
...
fn parse_pragma_directive<'i>(
    pair: Pair<'i, Rule>,
) -> Result<PragmaDirectiveAst<'i>, CompilerError> {
    let span = Span::from(pair.as_span());
    let mut inner = pair.into_inner();
    let name_pair = inner
        .next()
        .ok_or_else(|| CompilerError::Unsupported("missing pragma name".to_string()))?;
    let name_span = Span::from(name_pair.as_span());
    let name = name_pair.as_str().to_string();
    let value_pair = inner
        .next()
        .ok_or_else(|| CompilerError::Unsupported("missing pragma value".to_string()))?;
    let value_span = Span::from(value_pair.as_span());
    let value = value_pair.as_str().trim().to_string();

    Ok(PragmaDirectiveAst {
        name,
        value,
        span,
        name_span,
...
fn parse_contract_definition<'i>(
    ...
    let params_pair = inner
        .next()
        .ok_or_else(|| CompilerError::Unsupported("missing contract parameters".to_string()))?;
        ...
        if let Some(inner_item) = item_inner.next() {
            match inner_item.as_rule() {
                ...
                Rule::contract_field_definition => {
                    fields.push(parse_contract_field_definition(inner_item)?)
                }
...
fn parse_struct_definition<'i>(pair: Pair<'i, Rule>) -> Result<StructAst<'i>, CompilerError> {
    ...
    let name_pair = inner
        .next()
        .ok_or_else(|| CompilerError::Unsupported("missing struct name".to_string()))?;
    let Identifier {
        name,
        span: name_span,
    } = parse_identifier(name_pair)?;
...
fn parse_struct_field_definition<'i>(
    pair: Pair<'i, Rule>,
) -> Result<StructFieldAst<'i>, CompilerError> {
    let span = Span::from(pair.as_span());
    let mut inner = pair.into_inner();
    let type_pair = inner
        .next()
        .ok_or_else(|| CompilerError::Unsupported("missing struct field type".to_string()))?;
    let type_span = Span::from(type_pair.as_span());
    let type_ref = parse_type_name_pair(type_pair)?;
    let name_pair = inner
        .next()
        .ok_or_else(|| CompilerError::Unsupported("missing struct field name".to_string()))?;
    let Identifier {
        name,
        span: name_span,
    } = parse_identifier(name_pair)?;
...
fn parse_function_attribute<'i>(
    pair: Pair<'i, Rule>,
) -> Result<FunctionAttributeAst<'i>, CompilerError> {
    let span = Span::from(pair.as_span());
    let mut inner = pair.into_inner();

    let path_pair = inner
        .next()
        .ok_or_else(|| CompilerError::Unsupported("missing attribute path".to_string()))?;
...
fn parse_attribute_path<'i>(
    pair: Pair<'i, Rule>,
) -> Result<(Vec<String>, Vec<Span<'i>>), CompilerError> {
    ...
    if path.is_empty() {
        return Err(CompilerError::Unsupported(
            "attribute path must not be empty".to_string(),
        ));
    }
    Ok((path, spans))
}

fn parse_attribute_args<'i>(
    pair: Pair<'i, Rule>,
) -> Result<Vec<FunctionAttributeArgAst<'i>>, CompilerError> {
    if pair.as_rule() != Rule::attribute_args {
        return Err(CompilerError::Unsupported(
            "expected attribute arguments".to_string(),
        ));
    }
    let mut out = Vec::new();
    for inner in pair.into_inner() {
...
fn parse_attribute_arg<'i>(
    pair: Pair<'i, Rule>,
) -> Result<FunctionAttributeArgAst<'i>, CompilerError> {
    let span = Span::from(pair.as_span());
    ...
    let name_pair = inner
        .next()
        .ok_or_else(|| CompilerError::Unsupported("missing attribute argument name".to_string()))?;
    let expr_pair = inner.next().ok_or_else(|| {
        CompilerError::Unsupported("missing attribute argument value".to_string())
    })?;
...
fn parse_contract_field_definition<'i>(
    pair: Pair<'i, Rule>,
    ...
    let name_pair = inner
        .next()
        .ok_or_else(|| CompilerError::Unsupported("missing field name".to_string()))?;
    let expr_pair = inner
        .next()
        .ok_or_else(|| CompilerError::Unsupported("missing field initializer".to_string()))?;
    ...
    match pair.as_rule() {
        ...
        Rule::variable_definition => {
            ...
            let type_pair = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing variable type".to_string()).with_span(&span)
            })?;
            let type_span = Span::from(type_pair.as_span());
            ...
            let ident = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing variable name".to_string()).with_span(&span)
            })?;
            let Identifier {
                name,
                span: name_span,
            } = parse_identifier(ident).map_err(|err| err.with_span(&span))?;
            let expr = match inner.next() {
                Some(expr_pair) => {
                    Some(parse_expression(expr_pair).map_err(|err| err.with_span(&span))?)
                }
                None => None,
            };
        ...
        Rule::tuple_assignment => {
            let mut inner = pair.into_inner();
            let left_type_pair = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing left tuple type".to_string()).with_span(&span)
            })?;
            let left_ident = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing left tuple name".to_string()).with_span(&span)
            })?;
            let right_type_pair = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing right tuple type".to_string()).with_span(&span)
            })?;
            let right_ident = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing right tuple name".to_string()).with_span(&span)
            })?;
            let expr_pair = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing tuple expression".to_string()).with_span(&span)
            })?;
        ...
        Rule::function_call_assignment => {
            ...
            let call_pair = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing function call".to_string()).with_span(&span)
            })?;
            let (
                Identifier {
                    name,
        ...
        Rule::state_function_call_assignment => {
            ...
            while let Some(p) = inner.peek() {
                ...
                let binding = inner.next().expect("checked");
                bindings
                    .push(parse_state_typed_binding(binding).map_err(|err| err.with_span(&span))?);
            }
            let call_pair = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing function call".to_string()).with_span(&span)
            })?;
        ...
        Rule::struct_destructure_assignment => {
            ...
            let expr_pair = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing destructuring expression".to_string())
                    .with_span(&span)
            })?;
            let expr = parse_expression(expr_pair).map_err(|err| err.with_span(&span))?;
        ...
        Rule::call_statement => {
            let mut inner = pair.into_inner();
            let call_pair = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing call statement".to_string()).with_span(&span)
            })?;
            let (
                Identifier {
                    name,
        ...
        Rule::assign_statement => {
            let mut inner = pair.into_inner();
            let ident = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing assignment name".to_string()).with_span(&span)
            })?;
            let expr_pair = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing assignment expression".to_string())
                    .with_span(&span)
            })?;
            let expr = parse_expression(expr_pair).map_err(|err| err.with_span(&span))?;
            let Identifier {
        ...
        Rule::return_statement => {
            ...
            let value_pair = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing return values".to_string()).with_span(&span)
            })?;
            let exprs = match value_pair.as_rule() {
                Rule::expression_list => {
                    parse_expression_list(value_pair).map_err(|err| err.with_span(&span))?
                }
        ...
        Rule::time_op_statement => {
            let mut inner = pair.into_inner();
            let tx_var = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing time op variable".to_string()).with_span(&span)
            })?;
            let expr_pair = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing time op expression".to_string())
                    .with_span(&span)
            })?;
            let message = inner
                .next()
                .map(parse_require_message)
                .transpose()
            ...
            let tx_var_value = match tx_var.as_str() {
                ...
                other => {
                    return Err(CompilerError::Unsupported(format!(
                        "unsupported time variable: {other}"
                    ))
                    .with_span(&tx_var_span));
                }
            };
        ...
        Rule::if_statement => {
            ...
            let cond_pair = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing if condition".to_string()).with_span(&span)
            })?;
            let cond_expr = parse_expression(cond_pair).map_err(|err| err.with_span(&span))?;
            let then_block = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing if block".to_string()).with_span(&span)
            })?;
            let (then_branch, then_span) =
                parse_block(then_block).map_err(|err| err.with_span(&span))?;
            let else_data = inner
                .next()
                .map(parse_block)
                .transpose()
        ...
        Rule::for_statement => {
            let mut inner = pair.into_inner();
            let ident = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing for loop identifier".to_string())
                    .with_span(&span)
            })?;
            let start_pair = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing for loop start".to_string()).with_span(&span)
            })?;
            let end_pair = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing for loop end".to_string()).with_span(&span)
            })?;
            let max_iterations_pair = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing for loop max iterations".to_string())
                    .with_span(&span)
            })?;
            let block_pair = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing for loop body".to_string()).with_span(&span)
            })?;
            ...
            let max_iterations_expr =
                parse_expression(max_iterations_pair).map_err(|err| err.with_span(&span))?;
        ...
        Rule::console_statement => {
            ...
            let list_pair = inner.next().ok_or_else(|| {
                CompilerError::Unsupported("missing console arguments".to_string()).with_span(&span)
            })?;
            let args =
                parse_console_parameter_list(list_pair).map_err(|err| err.with_span(&span))?;
            Ok(Statement::Console { args, span })
        }
        _ => Err(
            CompilerError::Unsupported(format!("unexpected statement: {:?}", pair.as_rule()))
                .with_span(&span),
        ),
    }
}
    ...
    match pair.as_rule() {
        Rule::block => {
            let inner = pair.into_inner().next().ok_or_else(|| {
                CompilerError::Unsupported("empty block".to_string()).with_span(&span)
            })?;
            parse_block(inner)
        }
        Rule::braced_block => {
...
fn parse_require_message<'i>(pair: Pair<'i, Rule>) -> Result<(String, Span<'i>), CompilerError> {
    ...
    match parse_string_literal(inner)? {
        ...
        _ => Err(CompilerError::Unsupported(
            "require message must be a string literal".to_string(),
        )),
    }
}

fn parse_state_typed_binding<'i>(
    pair: Pair<'i, Rule>,
) -> Result<StructBindingAst<'i>, CompilerError> {
    let span = Span::from(pair.as_span());
    let mut inner = pair.into_inner();
...
fn parse_expression<'i>(pair: Pair<'i, Rule>) -> Result<Expr<'i>, CompilerError> {
    match pair.as_rule() {
        ...
        Rule::BooleanLiteral => Ok(Expr::new(
            ExprKind::Bool(pair.as_str() == "true"),
        ...
        | Rule::DateLiteral
        | Rule::type_name
        | Rule::state_entry => Err(CompilerError::Unsupported(format!(
...
fn parse_conditional<'i>(pair: Pair<'i, Rule>) -> Result<Expr<'i>, CompilerError> {
    ...
    let condition_pair = inner
        .next()
        .ok_or_else(|| CompilerError::Unsupported("missing conditional condition".to_string()))?;
    ...
    let else_pair = inner.next().ok_or_else(|| {
        CompilerError::Unsupported("missing conditional else expression".to_string())
    })?;
    ...
    for postfix in inner {
        ...
        match postfix.as_rule() {
            ...
            Rule::slice_call => {
                ...
                let start_expr = slice_inner
                    .next()
                    .ok_or_else(|| CompilerError::Unsupported("missing slice start".to_string()))?;
                let end_expr = slice_inner
                    .next()
                    .ok_or_else(|| CompilerError::Unsupported("missing slice end".to_string()))?;
            ...
            Rule::append_call => {
                ...
                let list_pair = append_inner.next().ok_or_else(|| {
                    CompilerError::Unsupported("missing append expressions".to_string())
                })?;
                let args = parse_expression_list(list_pair)?;
                if args.is_empty() {
                    return Err(CompilerError::Unsupported(
                        "append requires at least one expression".to_string(),
                    )
            ...
            Rule::tuple_index => {
                ...
                if matches!(&expr.kind, ExprKind::Split { .. }) {
                    return Err(CompilerError::Unsupported(
                        "split() results must be accessed with .0 or .1".to_string(),
                    )
                    .with_span(&postfix_span));
                }
                expr = Expr::new(
            ...
            Rule::tuple_field_access => {
                ...
                let index = raw.parse::<usize>().map_err(|_| {
                    CompilerError::Unsupported(format!("invalid tuple field index '{raw}'"))
                        .with_span(&postfix_span)
                })?;
                let span = expr.span.join(&postfix_span);
                if let ExprKind::Split {
                    source,
                    index: split_index,
                    span: split_span,
                    ..
                } = &expr.kind
                {
                    let part = match index {
                        ...
                        _ => {
                            return Err(CompilerError::Unsupported(
                                "split() index must be 0 or 1".to_string(),
                            )
                            .with_span(&postfix_span));
                        }
                    };
            ...
            Rule::field_access => {
                if matches!(&expr.kind, ExprKind::Introspection { .. })
                    || expr_root_identifier(&expr).as_deref() == Some("tx")
                {
                    return Err(CompilerError::Unsupported(
                        "field access on transaction introspection is not supported".to_string(),
                    ));
                }
                let field_pair = postfix.into_inner().next().ok_or_else(|| {
                    CompilerError::Unsupported("missing field access name".to_string())
                })?;
                let Identifier {
                    name: field,
                    span: field_span,
...
fn parse_typed_parameter_list<'i>(
    pair: Pair<'i, Rule>,
...
fn parse_return_type_list<'i>(
    pair: Pair<'i, Rule>,
) -> Result<(Vec<TypeRef>, Vec<Span<'i>>), CompilerError> {
    let mut return_types = Vec::new();
...
fn parse_state_object<'i>(pair: Pair<'i, Rule>) -> Result<Expr<'i>, CompilerError> {
    ...
    for field_pair in pair.into_inner() {
        ...
        let name_pair = inner
            ...
            .ok_or_else(|| CompilerError::Unsupported("missing state field name".to_string()))?;
        let expr_pair = inner.next().ok_or_else(|| {
            CompilerError::Unsupported("missing state field expression".to_string())
        })?;
        let Identifier {
            name,
            span: name_span,
...
fn parse_number(raw: &str) -> Result<i64, CompilerError> {
    ...
    if let Some(exp_raw) = exp_raw {
        ...
        let exp = exp_clean.parse::<u32>().map_err(|_| {
            CompilerError::InvalidLiteral(format!("invalid number literal '{raw}'"))
        })?;
        let pow = 10i128.checked_pow(exp).ok_or_else(|| {
            CompilerError::InvalidLiteral(format!("number literal overflow '{raw}'"))
        })?;
        value = value.checked_mul(pow).ok_or_else(|| {
            CompilerError::InvalidLiteral(format!("number literal overflow '{raw}'"))
        })?;
    }
...
fn parse_function_call_parts<'i>(
    pair: Pair<'i, Rule>,
) -> Result<(Identifier<'i>, Vec<Expr<'i>>), CompilerError> {
    let mut inner = pair.into_inner();
    let name_pair = inner
        .next()
        .ok_or_else(|| CompilerError::Unsupported("missing function name".to_string()))?;
...
fn parse_function_call<'i>(pair: Pair<'i, Rule>) -> Result<Expr<'i>, CompilerError> {
    ...
    let (
        ...
        args,
    ) = parse_function_call_parts(pair)?;
    Ok(Expr::new(
        ExprKind::Call {
            name,
            args,
...
fn parse_hex_literal<'i>(pair: Pair<'i, Rule>) -> Result<Expr<'i>, CompilerError> {
    ...
    Ok(Expr::new(
        ExprKind::Array(
            bytes
                .into_iter()
                .map(|byte| Expr::new(ExprKind::Byte(byte), span))
                .collect(),
        ),
        span,
    ))
...
fn apply_number_unit<'i>(expr: Expr<'i>, unit: &str) -> Result<Expr<'i>, CompilerError> {
    ...
    let value = match expr.kind {
        ...
        _ => {
            return Err(CompilerError::InvalidLiteral(
                "number literal is not an int".to_string(),
            ));
        }
    };
    let multiplier = match unit {
        ...
        _ => {
            return Err(CompilerError::Unsupported(format!(
                "number unit '{unit}' not supported"
            )));
        }
    };
    let scaled = value.checked_mul(multiplier).ok_or_else(|| {
        CompilerError::InvalidLiteral(format!("number literal overflow for unit '{unit}'"))
    })?;
...
fn parse_date_literal<'i>(pair: Pair<'i, Rule>) -> Result<Expr<'i>, CompilerError> {
    ...
    let string_pair = inner
        .next()
        .ok_or_else(|| CompilerError::InvalidLiteral("missing date literal".to_string()))?;
...
fn parse_introspection<'i>(pair: Pair<'i, Rule>) -> Result<Expr<'i>, CompilerError> {
    ...
    let kind = if text.starts_with("tx.inputs") {
        match field_raw {
            ...
            _ => {
                return Err(CompilerError::Unsupported(format!(
                    "input field '{field_raw}' not supported"
                )));
            }
        }
        ...
        match field_raw {
            ...
            _ => {
                return Err(CompilerError::Unsupported(format!(
                    "output field '{field_raw}' not supported"
                )));
            }
        }
    } else {
    ...
    Ok(Expr::new(
        ExprKind::Introspection {
            kind,
            index,
            field_span: Span::from(field_pair.as_span()),
        },
        span,
    ))
}
...
fn parse_identifier<'i>(pair: Pair<'i, Rule>) -> Result<Identifier<'i>, CompilerError> {
    ...
    if value.starts_with("__") {
        return Err(CompilerError::Unsupported(
            "identifiers starting with '__' are reserved".to_string(),
        ));
    }
