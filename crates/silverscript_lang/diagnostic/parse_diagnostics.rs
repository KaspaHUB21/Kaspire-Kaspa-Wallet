...
const INTERPRETATION_HEURISTICS: &[InterpretationHeuristic] = &[InterpretationHeuristic {
    interpretation: ParseErrorInterpretation::MissingSemicolon,
    matches: expects_semicolon,
}];
...
pub(crate) fn interpret_parse_error(
    input: &str,
    ...
    let mut diagnostic = ParseDiagnostic::new(interpretation, span, input, primary_message)
        .with_expected_tokens(normalize_expected_tokens(
            &attempt_data.expected_tokens,
            spec,
        ))
...
fn interpretation_spec(interpretation: ParseErrorInterpretation) -> &'static InterpretationSpec {
    INTERPRETATION_SPECS
        .iter()
        .find(|spec| spec.interpretation == interpretation)
        .unwrap_or(&UNCLASSIFIED_SPEC)
}
...
fn primary_labels(spec: &InterpretationSpec, span: ErrorSpan) -> Vec<ParseDiagnosticLabel> {
    spec.primary_label
        .map(|label| vec![ParseDiagnosticLabel::new(span, label)])
        .unwrap_or_default()
}
