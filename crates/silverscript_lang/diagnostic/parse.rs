...
impl ParseDiagnostic {
    ...
    pub fn display_location(&self) -> ParseDisplayLocation {
        ...
        let position = Position::new(&self.source_text, pos)
            .unwrap_or_else(|| Position::from_start(&self.source_text));
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if !self.expected_tokens.is_empty() {
            let expected_tokens = self
                .expected_tokens
                .iter()
                .map(|token| format_expected_token(token))
                .collect::<Vec<_>>()
                .join(", ");
            writeln!(
                f,
                "{spacing}   note: expected one of tokens: {expected_tokens}"
            )?;
        }
