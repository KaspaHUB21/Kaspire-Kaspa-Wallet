use crate::{
    derive_address_range, derive_backup_key, export_private_key, generate_wallet_with_passphrase, import_private_key,
    import_wallet_with_passphrase, prepare_inscription, prepare_kcc20_transfer, prepare_reveal,
    prepare_policy_transaction, prepare_transaction, sign_kcc20_transfer, sign_personal_message,
    sign_policy_transaction, sign_reveal, sign_transaction, InscriptionRequest,
    Kcc20TransferRequest, PolicyTransactionRequest, RevealRequest, SendRequest,
};
use jni::{
...
pub extern "system" fn Java_space_kasvault_wallet_SecureCore_signPolicyTransaction(
    ...
    let result = (|| {
        ...
        let request: PolicyTransactionRequest = serde_json::from_str(&request_json)
            .map_err(|_| "invalid policy transaction request".to_string())?;
        let signed = sign_policy_transaction(&secret, &request, &review_hash)
